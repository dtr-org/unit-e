...
AC_DEFUN([UNITE_QT_CONFIGURE],[
  ...
  if test "x$use_pkgconfig" = xyes; then
    UNITE_QT_CHECK([_UNITE_QT_FIND_LIBS_WITH_PKGCONFIG([$2])])
  else
  ...
  if test "x$unite_qt_got_major_vers" = x5; then
    _UNITE_QT_IS_STATIC
    ...
    if test "x$TARGET_OS" = xwindows; then
      AC_DEFINE(QT_STATICPLUGIN, 1, [Define this symbol if qt plugins are static])
      _UNITE_QT_CHECK_STATIC_PLUGINS([
  ...
  UNITE_QT_PATH_PROGS([MOC], [moc-qt${unite_qt_got_major_vers} moc${unite_qt_got_major_vers} moc], $qt_bin_path)
  UNITE_QT_PATH_PROGS([UIC], [uic-qt${unite_qt_got_major_vers} uic${unite_qt_got_major_vers} uic], $qt_bin_path)
  UNITE_QT_PATH_PROGS([RCC], [rcc-qt${unite_qt_got_major_vers} rcc${unite_qt_got_major_vers} rcc], $qt_bin_path)
  UNITE_QT_PATH_PROGS([LRELEASE], [lrelease-qt${unite_qt_got_major_vers} lrelease${unite_qt_got_major_vers} lrelease], $qt_bin_path)
  UNITE_QT_PATH_PROGS([LUPDATE], [lupdate-qt${unite_qt_got_major_vers} lupdate${unite_qt_got_major_vers} lupdate],$qt_bin_path, yes)
  ...
  AC_MSG_RESULT([$unite_enable_qt (Qt${unite_qt_got_major_vers})])
  ...
  AC_SUBST(QT_SELECT, qt${unite_qt_got_major_vers})
  AC_SUBST(MOC_DEFS)
])
...
dnl Inputs: unite_qt_got_major_vers. 4 or 5.
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       ...
       if test "x$TARGET_OS" = xwindows; then
         ...
         if test "x$unite_cv_need_platformsupport" = xyes; then
           UNITE_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}PlatformSupport],[main],,UNITE_QT_FAIL(lib${QT_LIB_PREFIX}PlatformSupport not found)))
         fi
       fi
     fi
  else
...
dnl Outputs: unite_qt_got_major_vers is set to "4" or "5".
...
AC_DEFUN([_UNITE_QT_FIND_LIBS_WITH_PKGCONFIG],[
  ...
  if test "x$auto_priority_version" = x; then
    ...
    if test "x$unite_qt_want_version" = xqt5 ||  ( test "x$unite_qt_want_version" = xauto && test "x$auto_priority_version" = xqt5 ); then
      QT_LIB_PREFIX=Qt5
      unite_qt_got_major_vers=5
    else
      QT_LIB_PREFIX=Qt
      unite_qt_got_major_vers=4
    fi
    ...
    UNITE_QT_CHECK([
      ...
      elif test "x$unite_qt_want_version" = xqt4 || ( test "x$unite_qt_want_version" = xauto && test "x$auto_priority_version" = xqt4 ); then
        PKG_CHECK_MODULES([QT4], [$qt4_modules], [QT_INCLUDES="$QT4_CFLAGS"; QT_LIBS="$QT4_LIBS" ; have_qt=yes], [have_qt=no])
      fi

      dnl qt version is set to 'auto' and the preferred version wasn't found. Now try the other.
      if test "x$have_qt" = xno && test "x$unite_qt_want_version" = xauto; then
        if test "x$auto_priority_version" = xqt5; then
          PKG_CHECK_MODULES([QT4], [$qt4_modules], [QT_INCLUDES="$QT4_CFLAGS"; QT_LIBS="$QT4_LIBS" ; have_qt=yes; QT_LIB_PREFIX=Qt; unite_qt_got_major_vers=4], [have_qt=no])
        else
          PKG_CHECK_MODULES([QT5], [$qt5_modules], [QT_INCLUDES="$QT5_CFLAGS"; QT_LIBS="$QT5_LIBS" ; have_qt=yes; QT_LIB_PREFIX=Qt5; unite_qt_got_major_vers=5], [have_qt=no])
        fi
      fi
...
AC_DEFUN([_UNITE_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  UNITE_QT_CHECK([
    ...
    if test "x$unite_cv_qt5" = xyes || test "x$unite_qt_want_version" = xqt5; then
      QT_LIB_PREFIX=Qt5
      unite_qt_got_major_vers=5
    else
      QT_LIB_PREFIX=Qt
  ...
  UNITE_QT_CHECK(AC_SEARCH_LIBS([hb_ot_tags_from_script] ,[qtharfbuzzng harfbuzz],,AC_MSG_WARN([libharfbuzz not found. Assuming qt has it built-in or support is disabled])))
