...
with the unit-e(s) being tested (using python's asyncore package); the other
...
behavior of a unit-e against 1 or more other unit-e instances. It should not be used
...
option to change the unit-e binary used by the test.  If using 2 or more nodes,
then `--refbinary` can be optionally used to change the unit-e that will be used
on nodes 2 and up.
...
Each `TestInstance` consists of:
  - A list of `[object, outcome, hash]` entries
    * `object` is a `CBlock`, `CTransaction`, or
    ...
    complete headers-chains when requested from the unit-e, to allow writing
    tests where blocks can be delivered out of order but still processed by
    headers-first unit-e's.
    * `outcome` is `True`, `False`, or `None`.  If `True`
    or `False`, the tip is compared with the expected tip -- either the
    block passed in, or the hash specified as the optional 3rd entry.  If
    `None` is specified, then the test will compare all the unit-e's
  ...
  - `sync_every_transaction`: `True/False`.  Analogous to
    ...
    then the contents of the entire mempool are compared across all unit-e
