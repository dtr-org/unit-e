...
from test_framework.messages import COrderedBlock
...
class SegWitTest(UnitETestFramework):
    ...
    def update_witness_block_with_transactions(self, block, tx_list, nonce=0):
        ...
        if isinstance(block, COrderedBlock):
            block.append_transactions(tx_list)
        else:
    ...
    def test_unnecessary_witness_before_segwit_activation(self):
        ...
        # Right now, united delays sending reject messages for blocks
    ...
    def test_witness_block_size(self):
        ...
        block = COrderedBlock(self.build_next_block())
        ...
        while additional_bytes > 0:
            ...
            block.ovtx[-1].wit.vtxinwit[int(i/(2*NUM_DROPS))].scriptWitness.stack[i%(2*NUM_DROPS)] = b'a'*(195+extra_bytes)
            additional_bytes -= extra_bytes
            i += 1

        block.ovtx[0].vout.pop()  # Remove old commitment
        add_witness_commitment(block)
        block.solve()
        vsize = get_virtual_size(block)
        ...
        cur_length = len(block.ovtx[-1].wit.vtxinwit[0].scriptWitness.stack[0])
        block.ovtx[-1].wit.vtxinwit[0].scriptWitness.stack[0] = b'a'*(cur_length-1)
        block.ovtx[0].vout.pop()
        add_witness_commitment(block)
        block.solve()
        ...
        self.utxo.pop(0)
        self.utxo.append(UTXO(block.ovtx[-1].sha256, 0, block.ovtx[-1].vout[0].nValue))
    ...
    def test_signature_version_1(self):
        ...
        # Remove the previously incorrect transaction
        block.remove_at(-1)
    ...
    def test_p2sh_witness(self, segwit_activated):
        ...
        # segwit activation.  Note that older united's that are not
    ...
    # To enable this test, pass --oldbinary=<path-to-pre-segwit-united> to
    # the test.
    def test_upgrade_after_activation(self, node_id):
        self.log.info("Testing software upgrade after softfork activation")

        assert(node_id != 0) # node0 is assumed to be a segwit-active united
