...
The RPM spec file provided here is for UnitE-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://unite.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[UnitE git repository](https://github.com/unite/unite) but are not part of
...
Currently only `Source10` is used, for the example `unite.conf` file.
...
in the unite git repository. At present only an SVG version of the UnitE
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the unite
...
needed when 0.12.1 is released, a proper fix is already in the UnitE git
master branch.
...
The `unite.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `unite-server` package. At this point, most RPM based
...
The `unite-server` package should have SELinux support. How to properly do
...
UnitE-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/unite/). He has been
packaging UnitE for Fedora at least since 2012.
...
4. I renamed the `unite` package that contains the Qt GUI to `unite-core` as
that appears to be how the general population refers to it, in contrast to
`unite-xt` or `unite-classic`. I wanted to make sure the general population
