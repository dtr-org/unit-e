...
class CInit {
 public:
  CInit() {
    ...
    // modules and engines. We don't use them so we don't require the config.
    // However some of our libs may call functions which attempt to load the
    // config file, possibly resulting in an exit() or crash if it is missing or
    // corrupt. Explicitly tell OpenSSL not to try to load the file. The result
    // for our libs will be that the config appears to have been loaded and
    // there are no modules/engines available.
    OPENSSL_no_config();
...
static std::string LogTimestampStr(const std::string& str,
  ...
  if (*fStartedNewLine) {
    ...
    if (fLogTimeMicros) strStamped += strprintf(".%06d", nTimeMicros % 1000000);
...
void ArgsManager::ReadConfigFile(const std::string& confPath) {
  ...
  if (!streamConfig.good()) return;  // No unite.conf file is OK
...
std::string CopyrightHolders(const std::string& strPrefix) {
  ...
  // Check for untranslated substitution to make sure UnitE Core copyright is
  // not removed by accident
  if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION)
          .find("UnitE Core") == std::string::npos) {
