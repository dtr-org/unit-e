...
class bit_packed_atomic_flags {
 ...
 public:
  ...
  explicit bit_packed_atomic_flags(uint32_t size) {
    ...
    for (uint32_t i = 0; i < size; ++i) mem[i].store(0xFF);
  };
...
class cache {
 private:
   ...
   * possible.  Ideally this would be done by bitmasking but the size is usually
   * not a power of two.
   *
   ...
   * it.); when the divisor is a constant the compiler will do clever tricks to
   * turn it into a multiply+add+shift, but size is a run-time value so the
   * compiler can't do that here.
   *
   * One option would be to implement the same trick the compiler uses and
   * compute the constants for exact division based on the size, as described in
   * "{N}-bit Unsigned Division via {N}-bit Multiply-Add" by Arch D. Robison in
   * 2005. But that code is somewhat complicated and the result is still slower
   * than other options:
   *
   * Instead we treat the 32-bit random number as a Q32 fixed-point number in
   * the range [0,1) and simply multiply it by the size.  Then we just shift the
   * result down by 32-bits to get our bucket number.  The results has
   ...
   * precision is required but for a 32-bit random number we only need the high
   * 32 bits of a 32*32->64 multiply, which means the operation is reasonably
   * fast even on a typical 32-bit processor.
   *
