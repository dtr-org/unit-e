...
class CTransaction {
 public:
   ...
   *
   * Change from bitcoin: The version field of transactions is unsigned in
   * UnitE. This is intentional. The actual version of a transaction are the two
   * lower bytes of the version field. UnitE distinguishes different types of
   * transactions. Thus the upper two bytes are used to piggy back that type
   * (also uint16_t) onto the version field.
   */
 ...
 public:
  ...
  //! Returns the version of this transaction (considers only the two lower
  //! bytes of nVersion).
  uint16_t GetVersion() const { return static_cast<uint16_t>(nVersion); }

  //! Returns the transaction type (TxType) of this transaction (stored in the
  //! two upper bytes of the nVersion field).
  ...
  bool IsCoinStake() const {
    // UNIT-E: TODO: Check whether we need this distinction or we can deduce
    // like Bitcoin's IsCoinBase() above
