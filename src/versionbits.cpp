...
int AbstractThresholdConditionChecker::GetStateSinceHeightFor(
  ...
  // A block's state is always the same as that of the first of its period, so
  // it is computed based on a pindexPrev whose height equals a multiple of
  // nPeriod - 1. To ease understanding of the following height calculation, it
  // helps to remember that right now pindexPrev points to the block prior to
  // the block that we are computing for, thus: if we are computing for the last
  // block of a period, then pindexPrev points to the second to last block of
  // the period, and if we are computing for the first block of a period, then
  // pindexPrev points to the last block of the previous period. The parent of
  // the genesis block is represented by nullptr.
  pindexPrev = pindexPrev->GetAncestor(pindexPrev->nHeight -
                                       ((pindexPrev->nHeight + 1) % nPeriod));
