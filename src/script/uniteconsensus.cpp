...
static int verify_script(const unsigned char* scriptPubKey,
  ...
  if (!verify_flags(flags)) {
    return uniteconsensus_ERR_INVALID_FLAGS;
  }
  try {
    ...
    return set_error(err,
                     uniteconsensus_ERR_TX_DESERIALIZE);  // Error deserializing
  }
}

int uniteconsensus_verify_script_with_amount(
...
int uniteconsensus_verify_script(const unsigned char* scriptPubKey,
