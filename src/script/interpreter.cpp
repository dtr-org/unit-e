...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack,
  ...
  try {
    while (pc < pend) {
      ...
      // With SCRIPT_VERIFY_CONST_SCRIPTCODE, OP_CODESEPARATOR in non-segwit
      // script is rejected even in an unexecuted branch
      if (opcode == OP_CODESEPARATOR && sigversion == SIGVERSION_BASE &&
      ...
      if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
        ...
        switch (opcode) {
          ...
          case OP_16: {
            ...
            // push the data they push, so no need for a CheckMinimalPush here.
          } break;
          ...
          case OP_CHECKSEQUENCEVERIFY: {
            ...
            if ((nSequence & CTxIn::SEQUENCE_LOCKTIME_DISABLE_FLAG) != 0) break;
          ...
          case OP_EQUALVERIFY:
            // case OP_NOTEQUAL: // use OP_NUMNOTEQUAL
            {
              ...
              // something like n != 1 and have some wiseguy pass in 1 with
              // extra zero bytes after it (numerically, 0x01 == 0x0001 ==
              // 0x000001)
          ...
          case OP_CHECKMULTISIGVERIFY: {
            ...
            if (nOpCount > MAX_OPS_PER_SCRIPT)
              return set_error(serror, SCRIPT_ERR_OP_COUNT);
            int ikey = ++i;
            // ikey2 is the position of last non-signature item in the stack.
            // Top stack item = 1. With SCRIPT_VERIFY_NULLFAIL, this is used for
            // cleanup if operation fails.
            ...
            while (fSuccess && nSigsCount > 0) {
              ...
              // evaluation distinguishable by CHECKMULTISIG NOT if the
              // STRICTENC flag is set. See the script_(in)valid tests for
              // details.
              if (!CheckSignatureEncoding(vchSig, flags, serror) ||
                  !CheckPubKeyEncoding(vchPubKey, flags, sigversion, serror)) {
                // serror is set
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey,
  ...
  if (flags & SCRIPT_VERIFY_WITNESS) {
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
      ...
      // not clean for witness programs.
      stack.resize(1);
    }
  }
  ...
  if (flags & SCRIPT_VERIFY_WITNESS) {
    ...
    // require that WITNESS implies P2SH. Otherwise, going from
...
size_t static WitnessSigOps(int witversion,
  ...
  if (witversion == 0) {
    if (witprogram.size() == 20) return 1;
