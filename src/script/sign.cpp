...
bool TransactionSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig,
  ...
  if (sigversion == SIGVERSION_WITNESS_V0 && !key.IsCompressed()) return false;
...
static bool Sign1(const CKeyID& address, const BaseSignatureCreator& creator,
  ...
  if (!creator.CreateSig(vchSig, address, scriptCode, sigversion)) return false;
...
static bool SignN(const std::vector<valtype>& multisigdata,
  ...
  for (unsigned int i = 1; i < multisigdata.size() - 1 && nSigned < nRequired;
    ...
    if (Sign1(keyID, creator, scriptCode, ret, sigversion)) ++nSigned;
  }
 ...
 * scriptSigRet is the redemption script. Returns false if scriptPubKey could
  ...
  switch (txType) {
    ...
    case TX_SCRIPTHASH:
      if (sigs1.script.empty() || sigs1.script.back().empty())
        ...
        Stacks result = CombineSignatures(
            pubKey2, checker, txType2, vSolutions2, sigs1, sigs2, sigversion);
    ...
    case TX_WITNESS_V0_SCRIPTHASH:
      if (sigs1.witness.empty() || sigs1.witness.back().empty())
        ...
        // Recur to combine:
        CScript pubKey2(sigs1.witness.back().begin(),
                        sigs1.witness.back().end());
        txnouttype txType2;
        ...
        sigs2.witness.clear();
        Stacks result =
            CombineSignatures(pubKey2, checker, txType2, vSolutions2, sigs1,
                              sigs2, SIGVERSION_WITNESS_V0);
...
bool IsSolvable(const CKeyStore& store, const CScript& script) {
  ...
  // transaction, we would still accept and relay that transaction. In
  // particular, it will reject witness outputs that require signing with an
  // uncompressed public key.
  DummySignatureCreator creator(&store);
  SignatureData sigs;
  // Make sure that STANDARD_SCRIPT_VERIFY_FLAGS includes
  // SCRIPT_VERIFY_WITNESS_PUBKEYTYPE, the most important property this function
