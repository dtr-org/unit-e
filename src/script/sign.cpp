                               ...
                               const uint160 &pub_key_hash, std::vector<valtype> &ret, SigVersion sigversion)
{
    ...
    if (Sign1(key_id, creator, script_pub_key, ret, sigversion)) {
        CPubKey key;
        creator.Provider().GetPubKey(key_id, key);
                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
    ...
    case TX_WITNESS_V1_REMOTE_STAKING:
        ...
        return SignWithPubKeyHash(creator, witnessscript, h160, ret, SigVersion::WITNESS_V0);
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, const CTransaction* tx)
{
    ...
    bool solved = SignStep(creator, script, result, whichType, SigVersion::BASE);
    bool P2SH = false;
    CScript subscript;
