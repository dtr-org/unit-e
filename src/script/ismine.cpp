...
isminetype IsMine(const CKeyStore& keystore, const CScript& scriptPubKey,
  ...
  if (!Solver(scriptPubKey, whichType, vSolutions)) {
    if (keystore.HaveWatchOnly(scriptPubKey)) return ISMINE_WATCH_UNSOLVABLE;
    return ISMINE_NO;
  }
  ...
  switch (whichType) {
    ...
    case TX_WITNESS_V0_KEYHASH: {
      if (!keystore.HaveCScript(
              CScriptID(CScript() << OP_0 << vSolutions[0]))) {
        // We do not support bare witness outputs unless the P2SH version of it
        // would be acceptable as well. This protects against matching before
        // segwit activates. This also applies to the P2WSH case.
        break;
      }
