...
bool WalletParameterInteraction() {
  ...
  if (zapwallettxes && gArgs.SoftSetBoolArg("-persistmempool", false)) {
    LogPrintf(
        "%s: parameter interaction: -zapwallettxes enabled -> setting "
        "-persistmempool=0\n",
        __func__);
  }
  ...
  if (gArgs.GetBoolArg("-sysperms", false)) {
    return InitError(
        "-sysperms is not allowed in combination with enabled wallet "
        "functionality");
  }
  ...
  if (gArgs.IsArgSet("-maxtxfee")) {
    ...
    if (nMaxFee > HIGH_MAX_TX_FEE) {
      InitWarning(
          _("-maxtxfee is set very high! Fees this large could be paid on a "
            "single transaction."));
    }
    maxTxFee = nMaxFee;
