...
UniValue getnewaddress(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "set by -addresstype.\n"
        "\nResult:\n"
        "\"address\"    (string) The new unite address\n"
        "\nExamples:\n" +
...
UniValue getaccountaddress(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Returns the current UnitE address for receiving "
        ...
        "given name.\n"
        "\nResult:\n"
        "\"address\"          (string) The account unite address\n"
        "\nExamples:\n" +
...
UniValue setaccount(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The unite address to be "
...
UniValue getaccount(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The unite address for "
        "account lookup.\n"
        "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "  \"address\"         (string) a unite address associated with the "
        "given account\n"
        "  ,...\n"
        "]\n"
        "\nExamples:\n" +
...
UniValue sendtoaddress(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"            (string, required) The unite address to "
        "send to.\n"
        ...
        "                             The recipient will receive less unites "
...
UniValue signmessage(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() != 2)
    throw std::runtime_error(
        ...
        "\n"
        "\nArguments:\n"
        "1. \"address\"         (string, required) The unite address to use "
        "for the private key.\n"
        ...
        HelpExampleCli("verifymessage",
                       "\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XX\" \"signature\" "
                       "\"my message\"") +
        "\nAs json rpc\n" +
        HelpExampleRpc(
            "signmessage",
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request) {
  ...
  int nMinDepth = 1;
  if (!request.params[1].isNull()) nMinDepth = request.params[1].get_int();
...
UniValue getbalance(const JSONRPCRequest& request) {
  ...
  int nMinDepth = 1;
  if (!minconf.isNull()) nMinDepth = minconf.get_int();
  isminefilter filter = ISMINE_SPENDABLE;
  if (!include_watchonly.isNull())
    if (include_watchonly.get_bool()) filter = filter | ISMINE_WATCH_ONLY;
...
UniValue movecmd(const JSONRPCRequest& request) {
  ...
  if (!request.params[4].isNull()) strComment = request.params[4].get_str();
...
UniValue sendfrom(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
    throw std::runtime_error(
        ...
        "                       the spend.\n"
        "2. \"toaddress\"         (string, required) The unite address to send "
        "funds to.\n"
        ...
        HelpExampleCli("sendfrom",
                       "\"tabby\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.01 "
  ...
  if (nAmount <= 0)
    throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount for send");
  int nMinDepth = 1;
  if (!request.params[3].isNull()) nMinDepth = request.params[3].get_int();
...
UniValue sendmany(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "    {\n"
        "      \"address\":amount   (numeric or string) The unite address is "
        "the key, the numeric amount (can be string) in " +
        CURRENCY_UNIT +
        " is the value\n"
        "      ,...\n"
        "    }\n"
        ...
        "                           Those recipients will receive less unites "
        "than you enter in their corresponding amount field.\n"
  ...
  int nMinDepth = 1;
  if (!request.params[2].isNull()) nMinDepth = request.params[2].get_int();
  ...
  for (const std::string& name_ : keys) {
    ...
    if (!IsValidDestination(dest)) {
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         std::string("Invalid UnitE address: ") + name_);
    }
...
UniValue addmultisigaddress(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
    std::string msg =
        ...
        "of unite addresses or hex-encoded public keys\n"
        "     [\n"
        "       \"address\"                  (string) unite address or "
        ...
        "start united with -deprecatedrpc=addmultisigaddress).\n"
        "        clients should transition to the new output api before "
        "upgrading to v0.17.\n"
        "\"address\"                         (string) A unite address "
...
UniValue addwitnessaddress(const JSONRPCRequest& request) {
  ...
  if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
    throw JSONRPCError(
        ...
        "To use addwitnessaddress in v0.16, restart united with "
  ...
  if (w.already_witness) {
    ...
    pwallet->AddCScript(
        witprogram);  // Implicit for single-key now, but necessary for multisig
...
UniValue ListReceived(CWallet* const pwallet, const UniValue& params,
  ...
  // Minimum confirmations
  int nMinDepth = 1;
  if (!params[0].isNull()) nMinDepth = params[0].get_int();

  // Whether to include empty accounts
  bool fIncludeEmpty = false;
  if (!params[1].isNull()) fIncludeEmpty = params[1].get_bool();
...
UniValue listtransactions(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "                                                It will be \"\" for "
        "the default account.\n"
        "    \"address\":\"address\",    (string) The unite address of the "
  ...
  std::string strAccount = "*";
  if (!request.params[0].isNull()) strAccount = request.params[0].get_str();
  int nCount = 10;
  if (!request.params[1].isNull()) nCount = request.params[1].get_int();
  int nFrom = 0;
  if (!request.params[2].isNull()) nFrom = request.params[2].get_int();
  isminefilter filter = ISMINE_SPENDABLE;
  if (!request.params[3].isNull())
...
UniValue listaccounts(const JSONRPCRequest& request) {
  ...
  int nMinDepth = 1;
  if (!request.params[0].isNull()) nMinDepth = request.params[0].get_int();
...
UniValue listsinceblock(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        HelpExampleCli("listsinceblock",
                       "\"000000000000000bacf66f7497b7dc45ef753ee9a7d38571037cd"
                       "b1a57f663ad\" 6") +
  ...
  const CBlockIndex* pindex =
      nullptr;  // Block index of the specified block or the common ancestor, if
                // the block provided was in a deactivated chain.
  const CBlockIndex* paltindex =
      nullptr;  // Block index of the specified block, even if it's in a
                // deactivated chain.
  ...
  while (include_removed && paltindex && paltindex != pindex) {
    ...
    for (const CTransactionRef& tx : block.vtx) {
      ...
      if (it != pwallet->mapWallet.end()) {
        ...
        // here, even negative confirmation ones, hence the big negative.
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        HelpExampleCli("gettransaction",
                       "\"1075db55d416d3ca199f55b6084e2115b9345e16c5cf302fc80e9"
                       "d5fbf5d48d\"") +
        HelpExampleCli("gettransaction",
                       "\"1075db55d416d3ca199f55b6084e2115b9345e16c5cf302fc80e9"
                       "d5fbf5d48d\" true") +
...
UniValue walletpassphrase(const JSONRPCRequest& request) {
  ...
  // request.params[0] mlock()'d to begin with.
  ...
  // Clamp timeout
  constexpr int64_t MAX_SLEEP_TIME =
      100000000;  // larger values trigger a macos/libevent bug?
...
UniValue encryptwallet(const JSONRPCRequest& request) {
  ...
  StartShutdown();
  return "wallet encrypted; UnitE server stopping, restart to run with "
...
UniValue lockunspent(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "selection, when spending unites.\n"
...
UniValue listunspent(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "confirmations to filter\n"
        "3. \"addresses\"      (string) A json array of unite addresses to "
        "filter\n"
        "    [\n"
  ...
  if (!request.params[2].isNull()) {
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
      ...
      if (!IsValidDestination(dest)) {
        throw JSONRPCError(
            RPC_INVALID_ADDRESS_OR_KEY,
            std::string("Invalid UnitE address: ") + input.get_str());
      }
...
UniValue fundrawtransaction(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "address) The unite address to receive the change\n"
        ...
        "unites than you enter in their corresponding amount field.\n"
    ...
    if (request.params[1].type() == UniValue::VBOOL) {
      ...
      RPCTypeCheckObj(
          options,
          {
              ...
              {"reserveChangeKey",
               UniValueType(
                   UniValue::VBOOL)},  // DEPRECATED (and ignored), should be
                                       // removed in 0.16 or so.
...
UniValue rescanblockchain(const JSONRPCRequest& request) {
  ...
  if (!stopBlock) {
    ...
    // if we got a nullptr returned, ScanForWalletTransactions did rescan up to
    // the requested stopindex
