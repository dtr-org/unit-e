...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue,
  ...
  try {
    ...
    if (strType == "name") {
      ...
      // checksum. Newer wallets store keys as "key"[pubkey] =>
      // [privkey][hash(pubkey,privkey)], which is much faster while remaining
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet) {
  ...
  for (uint256 hash : wss.vWalletUpgrade) WriteTx(pwallet->mapWallet[hash]);
...
bool CWalletDB::RecoverKeysOnlyFilter(void* callbackData, CDataStream ssKey,
  ...
  if (!IsKeyType(strType) && strType != "hdchain") return false;
  if (!fReadOK) {
