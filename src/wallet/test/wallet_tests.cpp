...
BOOST_AUTO_TEST_CASE(coin_selection_tests) {
  ...
  for (int i = 0; i < RUN_TESTS; i++) {
    ...
    BOOST_CHECK(!testWallet.SelectCoinsMinConf(1 * EEES, 1, 6, 0, vCoins,
    ...
    BOOST_CHECK(testWallet.SelectCoinsMinConf(1 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    BOOST_CHECK(!testWallet.SelectCoinsMinConf(3 * EEES, 1, 6, 0, vCoins,
                                               setCoinsRet, nValueRet));

    // we can make 3 cents of new  coins
    BOOST_CHECK(testWallet.SelectCoinsMinConf(3 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    BOOST_CHECK(!testWallet.SelectCoinsMinConf(38 * EEES, 1, 6, 0, vCoins,
                                               setCoinsRet, nValueRet));
    // we can't even make 37 cents if we don't allow new coins even if they're
    // from us
    BOOST_CHECK(!testWallet.SelectCoinsMinConf(38 * EEES, 6, 6, 0, vCoins,
                                               setCoinsRet, nValueRet));
    // but we can make 37 cents if we accept new coins from ourself
    BOOST_CHECK(testWallet.SelectCoinsMinConf(37 * EEES, 1, 6, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 37 * EEES);
    // and we can make 38 cents if we accept all new coins
    BOOST_CHECK(testWallet.SelectCoinsMinConf(38 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 38 * EEES);

    // try making 34 cents from 1,2,5,10,20 - we can't do it exactly
    BOOST_CHECK(testWallet.SelectCoinsMinConf(34 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 35 * EEES);  // but 35 cents is closest
    BOOST_CHECK_EQUAL(setCoinsRet.size(),
                      3U);  // the best should be 20+10+5.  it's incredibly
                            // unlikely the 1 or 2 got included (but possible)

    // when we try making 7 cents, the smaller coins (1,2,5) are enough.  We
    // should see just 2+5
    BOOST_CHECK(testWallet.SelectCoinsMinConf(7 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 7 * EEES);
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 2U);

    // when we try making 8 cents, the smaller coins (1,2,5) are exactly enough.
    BOOST_CHECK(testWallet.SelectCoinsMinConf(8 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    BOOST_CHECK(testWallet.SelectCoinsMinConf(9 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    BOOST_CHECK(testWallet.SelectCoinsMinConf(71 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK(!testWallet.SelectCoinsMinConf(72 * EEES, 1, 1, 0, vCoins,
                                               setCoinsRet, nValueRet));

    // now try making 16 cents.  the best smaller coins can do is 6+7+8 = 21;
    // not as good at the next biggest coin, 20
    BOOST_CHECK(testWallet.SelectCoinsMinConf(16 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    BOOST_CHECK(testWallet.SelectCoinsMinConf(11 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    add_coin(3 * UNIT);
    add_coin(
        4 * UNIT);  // now we have 5+6+7+8+18+20+30+100+200+300+400 = 1094 cents
    BOOST_CHECK(testWallet.SelectCoinsMinConf(95 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet, 1 * UNIT);  // we should get 1 UNT in 1 coin
    BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

    BOOST_CHECK(testWallet.SelectCoinsMinConf(195 * EEES, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    ...
    // with 50k in change
    empty_wallet();
    for (int j = 0; j < 20; j++) add_coin(50000 * UNIT);

    BOOST_CHECK(testWallet.SelectCoinsMinConf(500000 * UNIT, 1, 1, 0, vCoins,
                                              setCoinsRet, nValueRet));
    BOOST_CHECK_EQUAL(nValueRet,
                      500000 * UNIT);  // we should get the exact amount
    ...
    for (CAmount amt = 1500; amt < UNIT; amt *= 10) {
      ...
      // per input)
      for (uint16_t j = 0; j < 676; j++) add_coin(amt);
    ...
    {
      empty_wallet();
      for (int i2 = 0; i2 < 100; i2++) add_coin(UNIT);

      // picking 50 from 100 coins doesn't depend on the shuffle,
      // but does depend on randomness in the stochastic approximation code
      BOOST_CHECK(testWallet.SelectCoinsMinConf(50 * UNIT, 1, 6, 0, vCoins,
                                                setCoinsRet, nValueRet));
      BOOST_CHECK(testWallet.SelectCoinsMinConf(50 * UNIT, 1, 6, 0, vCoins,
      ...
      for (int j = 0; j < RANDOM_REPEATS; j++) {
        ...
        // test will fail 1% of the time run the test RANDOM_REPEATS times and
        // only complain if all of them fail
        BOOST_CHECK(testWallet.SelectCoinsMinConf(UNIT, 1, 6, 0, vCoins,
                                                  setCoinsRet, nValueRet));
        BOOST_CHECK(testWallet.SelectCoinsMinConf(UNIT, 1, 6, 0, vCoins,
                                                  setCoinsRet2, nValueRet));
      ...
      for (int j = 0; j < RANDOM_REPEATS; j++) {
        ...
        // test will fail 1% of the time run the test RANDOM_REPEATS times and
        // only complain if all of them fail
        BOOST_CHECK(testWallet.SelectCoinsMinConf(90 * EEES, 1, 6, 0, vCoins,
                                                  setCoinsRet, nValueRet));
        BOOST_CHECK(testWallet.SelectCoinsMinConf(90 * EEES, 1, 6, 0, vCoins,
                                                  setCoinsRet2, nValueRet));
        if (equal_sets(setCoinsRet, setCoinsRet2)) fails++;
...
BOOST_AUTO_TEST_CASE(ApproximateBestSubset) {
  ...
  for (int i = 0; i < 1000; i++) add_coin(1000 * UNIT);
  add_coin(3 * UNIT);

  BOOST_CHECK(testWallet.SelectCoinsMinConf(1003 * UNIT, 1, 6, 0, vCoins,
                                            setCoinsRet, nValueRet));
  ...
  // after.
  {
    ...
    BOOST_CHECK_EQUAL(
        ...
        strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":"
                  ...
                  "by pruning or data corruption (see united log for details) "
                  "and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup) {
  ...
  // pubkey.
  AddTx(CRecipient{GetScriptForRawPubKey({}), 1 * UNIT,
                   false /* subtract fee */});
