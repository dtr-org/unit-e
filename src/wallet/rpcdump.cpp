...
UniValue importprivkey(const JSONRPCRequest& request) {
  ...
  bool fRescan = true;
  {
    ...
    std::string strLabel = "";
    if (!request.params[1].isNull()) strLabel = request.params[1].get_str();

    // Whether to perform rescan after import
    if (!request.params[2].isNull()) fRescan = request.params[2].get_bool();
...
UniValue importaddress(const JSONRPCRequest& request) {
  ...
  bool fP2SH = false;
  if (!request.params[3].isNull()) fP2SH = request.params[3].get_bool();

  {
    ...
    if (IsValidDestination(dest)) {
      ...
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         "Invalid UnitE address or script");
    }
  }
  if (fRescan) {
    pwallet->RescanFromTime(TIMESTAMP_MIN, reserver, true /* update */);
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        HelpExampleCli("removeprunedfunds",
                       "\"a8d0c0184dde994a09ec054286f1ce581bebf46446a512166eae7"
                       "628734ea0a5\"") +
        "\nAs a JSON-RPC call\n" +
  ...
  bool fGood = true;
  {
    ...
    while (file.good()) {
      pwallet->ShowProgress(
          "",
          std::max(
              1, std::min(99, (int)(((double)file.tellg() / (double)nFilesize) *
                                    100))));
      std::string line;
      ...
      if (vchSecret.SetString(vstr[0])) {
        ...
        if (fLabel) pwallet->SetAddressBook(keyid, strLabel, "receive");
...
UniValue dumpprivkey(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"   (string, required) The unite address for the private "
        "key\n"
        "\nResult:\n"
  ...
  return CUnitESecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "absolute or relative to united)\n"
        "\nResult:\n"
        "{                           (json object)\n"
   ...
   * https://github.com/unite/unite/issues/9934
  ...
  // produce output
  file << strprintf("# Wallet dump created by UnitE %s\n", CLIENT_BUILD);
  ...
  for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it =
    ...
    if (pwallet->GetKey(keyid, key)) {
      file << strprintf("%s %s ", CUnitESecret(key).ToString(), strTime);
...
UniValue importmulti(const JSONRPCRequest& mainRequest) {
  ...
  if (fRescan && fRunScan && requests.size()) {
    int64_t scannedTime =
        pwallet->RescanFromTime(nLowestTimestamp, reserver, true /* update */);
    pwallet->ReacceptWalletTransactions();
          ...
          result.pushKV(
              ...
              JSONRPCError(
                  ...
                  strprintf("Rescan failed for key with creation timestamp %d. "
                            ...
                            "caused by pruning or data corruption (see united "
                            "log for details) and could "
