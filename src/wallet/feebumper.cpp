...
static feebumper::Result PreconditionChecks(const CWallet* wallet,
  ...
  if (wtx.GetDepthInMainChain() != 0) {
    errors.push_back(
        "Transaction has been mined, or is conflicted with a mined "
        "transaction");
...
Result CreateTransaction(const CWallet* wallet, const uint256& txid,
  ...
  if (total_fee > 0) {
    ...
    // initialized in that unit (fee per kb). However, nOldFeeRate is a
    // calculated value from the tx fee/size, so add 1 satoshi to the result,
    // because it may have been rounded down.
  ...
  if (new_fee > maxTxFee) {
    errors.push_back(
        strprintf("Specified or calculated fee %s is too high (cannot be "
                  "higher than maxTxFee %s)",
  ...
  // the mempool) This may occur if the user set TotalFee or paytxfee too low,
  // if fallbackfee is too low, or, perhaps, in a rare situation where the
  // mempool minimum fee increased significantly since the fee estimation just a
  // moment earlier. In this case, we report an error to the user, who may use
  // total_fee to make an adjustment.
...
Result CommitTransaction(CWallet* wallet, const uint256& txid,
  ...
  if (!wallet->MarkReplaced(oldWtx.GetHash(), wtxBumped.GetHash())) {
    ...
    errors.push_back(
        "Created new bumpfee transaction but could not mark the original "
        "transaction as replaced");
  }
  return Result::OK;
}
