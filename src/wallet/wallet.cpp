...
void CWallet::DeriveNewChildKey(CWalletDB& walletdb, CKeyMetadata& metadata,
  ...
  do {
    ...
    // child-index-range example: 1 | BIP32_HARDENED_KEY_LIMIT == 0x80000001 ==
    // 2147483649
    if (internal) {
...
bool CWallet::LoadCScript(const CScript& redeemScript) {
  ...
  if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
    ...
    LogPrintf(
        "%s: Warning: This wallet contains a redeemScript of size %i which "
        "exceeds maximum size %i thus can never be redeemed. Do not use "
        "address %s.\n",
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase) {
...

  {
    ...
    if (!EncryptKeys(_vMasterKey)) {
      ...
      // not... die and let the user reload the unencrypted wallet.
      assert(false);
    }
...
bool CWallet::GetAccountDestination(CTxDestination& dest,
  ...
  if (!bForceNew) {
    if (!account.vchPubKey.IsValid()) {
      ...
      // Check if the current key has been used (TODO: check other addresses
      // with the same key)
      ...
      for (const CTxOut& txout : tx.vout) {
        ...
        for (const CKeyID& keyid : vAffected) {
          ...
          if (mi != m_pool_key_to_index.end()) {
            LogPrintf(
                "%s: Detected a used keypool key, mark all keypool key up to "
                "this key as used\n",
                __func__);
...
CBlockIndex* CWallet::ScanForWalletTransactions(
  ...
  CBlockIndex* ret = nullptr;
  {
    ...
    ShowProgress(_("Rescanning..."),
                 0);  // show rescan progress in GUI as dialog or on
...
std::vector<uint256> CWallet::ResendWalletTransactionsBefore(
...
OutputType CWallet::TransactionChangeType(
  ...
  unsigned int nBytes;
  {
    ...
    LOCK2(cs_main, cs_wallet);
    {
      ...
      if (!boost::get<CNoDestination>(&coin_control.destChange)) {
        ...
        //  be lost if a backup is restored, if the backup doesn't have the new
        //  private key for the change. If we reused the old key, it would be
        //  possible to add code to look for and rediscover unknown transactions
        //  that were written with keys of ours to recover post-backup change.
      ...
      while (true) {
          ...
          if (IsDust(txout, ::dustRelayFee)) {
            if (recipient.fSubtractFeeFromAmount && nFeeRet > 0) {
              if (txout.nValue < 0) {
                ...
                strFailReason =
                    _("The transaction amount is too small to send after the "
        ...
        // the next pass, give up because we must be at the maximum allowed fee.
                                ...
                                CConnman* connman, CValidationState& state) {
  {
    ...
    if (fBroadcastTransactions) {
      ...
      if (!wtx.AcceptToMemoryPool(maxTxFee, state)) {
        LogPrintf(
            "CommitTransaction(): Transaction cannot be broadcast immediately, "
            "%s\n",
...
std::set<CTxDestination> CWallet::GetAccountAddresses(
...
void CWallet::MarkReserveKeysAsUsed(int64_t keypool_id) {
  ...
  while (it != std::end(*setKeyPool)) {
    ...
    CKeyPool keypool;
    if (walletdb.ReadPool(index, keypool)) {  // TODO: This should be
                                              // unnecessary
      m_pool_key_to_index.erase(keypool.vchPubKey.GetID());
    }
...
void CWallet::GetKeyBirthTimes(
  ...
  CBlockIndex* pindexMax = chainActive[std::max(
      0, chainActive.Height() - 144)];  // the tip can be reorganized; use a
                                        // 144-block safety margin
  ...
  for (const auto& entry : mapKeyFirstBlock) {
    mapKeyBirth[entry.first] = entry.second->GetBlockTime() -
...
std::vector<std::string> CWallet::GetDestValues(
...
CWallet* CWallet::CreateWalletFromFile(
    const esperanza::Settings& esperanzaSettings,
  ...
  if (gArgs.GetBoolArg("-zapwallettxes", false)) {
    ...
    std::unique_ptr<CWallet> tempWallet =
        MakeUnique<CWallet>(esperanzaSettings, std::move(dbw));
  ...
  if (chainActive.Tip() && chainActive.Tip() != pindexRescan) {
    ...
    if (fPruneMode) {
      ...
      if (pindexRescan != block) {
        InitError(
            _("Prune: last wallet synchronisation goes beyond pruned data. You "
              "need to -reindex (download the whole blockchain again in case "
              "of pruned node)"));
        return nullptr;
      }
    }
...
bool CWalletTx::AcceptToMemoryPool(const CAmount& nAbsurdFee,
  ...
  if (mempool.exists(tx->GetHash())) {
    return state.Error(
        strprintf("%s: Transaction already in the mempool", __func__));
  }
...
CTxDestination CWallet::AddAndGetDestinationForScript(const CScript& script,
  ...
  switch (type) {
    ...
    case OUTPUT_TYPE_BECH32: {
      ...
      CScript witprog = GetScriptForDestination(witdest);
      // Check if the resulting program is solvable (i.e. doesn't use an
      // uncompressed key)
      if (!IsSolvable(*this, witprog)) {
        return CScriptID(script);
      }
      // Add the redeemscript, so that P2WSH and P2SH-P2WSH outputs are
      // recognized as ours.
      AddCScript(witprog);
