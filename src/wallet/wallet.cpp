...
CPubKey CWallet::DeriveNewPubKey(CWalletDB &walletdb, bool internal)
{
    AssertLockHeld(cs_wallet); // mapKeyMetadata
    ...
    if (!walletdb.WriteKeyMetadata(pubkey, metadata)) {
...
bool CWallet::LoadKeyMetadata(const CPubKey& pubkey, const CKeyMetadata &meta)
{
    ...
    return LoadKeyMetadata(keyID, meta);
}
...
{
    return !hdChain.seed_id.IsNull() || !hdChain.master_key_id.IsNull();
}
...
CAmount CWalletTx::GetAvailableWatchOnlyCredit(const bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++)
    {
        if (!pwallet->IsSpent(GetHash(), i)) {
            const CBlockIndex *block = nullptr;
            if (IsCoinBase() && GetBlocksToRewardMaturity(block) > 0) {
                const std::size_t num_of_rewards = GetComponent<proposer::FinalizationRewardLogic>()->GetNumberOfRewardOutputs(static_cast<blockchain::Height>(block->nHeight)) + 1;
                if (i < num_of_rewards) {
                    continue;
                }
            }
    ...
    unsigned int nBytes;
    {
        ...
        if (sign) {
            ...
            for (const auto& coin : setCoins) {
                ...
                std::unique_ptr<BaseSignatureCreator> txCreator(new TransactionSignatureCreator(
                    &txNewConst, nIn, coin.txout.nValue, SIGHASH_ALL
                ));
...
{
    {
        ...
        for (int64_t i = missingInternal + missingExternal - 1; i >= 0; --i) {
            ...
            if (IsHDEnabled() && hdChain.is_hardware_device) {
                ...
                pubkey = DeriveNewPubKey(walletdb, internal);
            } else {
                pubkey = GenerateNewKey(walletdb, internal);
            }
...
int CMerkleTx::GetBlocksToRewardMaturity(const CBlockIndex* &index_ret) const
{
    ...
    return std::max(0, (COINBASE_MATURITY+1) - GetDepthInMainChain(index_ret));
}
