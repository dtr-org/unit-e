...
void CDBEnv::EnvShutdown() {
  ...
  if (ret != 0)
    LogPrintf(
        "CDBEnv::EnvShutdown: Error %d shutting down database environment: "
        "%s\n",
...
bool CDBEnv::Open(const fs::path& pathIn, bool retry) {
  ...
  if (!LockDirectory(pathIn, ".walletlock")) {
    LogPrintf(
        "Cannot obtain a lock on wallet directory %s. Another instance of "
        "unite may be using it.\n",
        strPath);
    return false;
  }
...
bool CDB::Recover(const std::string& filename, void* callbackDataIn,
  ...
  for (CDBEnv::KeyValPair& row : salvagedData) {
    if (recoverKVcallback) {
      ...
      if (!(*recoverKVcallback)(callbackDataIn, ssKey, ssValue)) continue;
    }
...
bool CDBEnv::Salvage(const std::string& strFile, bool fAggressive,
  ...
  if (result == DB_VERIFY_BAD) {
    LogPrintf(
        "CDBEnv::Salvage: Database salvage found errors, all data may not be "
        "recoverable.\n");
    if (!fAggressive) {
      LogPrintf(
          "CDBEnv::Salvage: Rerun with aggressive mode to ignore errors and "
          "continue.\n");
      return false;
    }
  }
  ...
  while (!strDump.eof() && keyHex != DATA_END) {
    ...
    if (keyHex != DATA_END) {
      ...
      if (valueHex == DATA_END) {
        LogPrintf(
            "CDBEnv::Salvage: WARNING: Number of keys in data does not match "
            "number of values.\n");
        break;
      }
  ...
  if (keyHex != DATA_END) {
    LogPrintf(
        "CDBEnv::Salvage: WARNING: Unexpected end of file while reading "
        "salvage output.\n");
    return false;
  }
  ...
  while (true) {
    {
      ...
      if (!env->mapFileUseCount.count(strFile) ||
        ...
        if (fs::is_directory(pathDest)) pathDest /= strFile;
