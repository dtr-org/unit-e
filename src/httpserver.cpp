...
static bool HTTPBindAddresses(struct evhttp* http) {
  ...
  if (!gArgs.IsArgSet(
    ...
    if (gArgs.IsArgSet("-rpcbind")) {
      LogPrintf(
          "WARNING: option -rpcbind was ignored because -rpcallowip was not "
...
void StopHTTPServer() {
  ...
  if (eventBase) {
    ...
    // then break it Before this was solved with event_base_loopexit, but that
    // didn't work as expected in at least libevent 2.0.21 and always introduced
    // a delay. In libevent master that appears to be solved, so in the future
    // that solution could be used again (if desirable). (see discussion in
    // https://github.com/unite/unite/pull/6990)
    if (threadResult.valid() && threadResult.wait_for(std::chrono::milliseconds(
                                    2000)) == std::future_status::timeout) {
      LogPrintf(
          "HTTP event loop did not exit within allotted time, sending "
          "loopbreak\n");
