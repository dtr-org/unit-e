...
bool CTxMemPool::addUnchecked(const uint256 &hash, const CTxMemPoolEntry &entry,
  ...
  // UpdateTransactionsFromBlock to clean up the mess we're leaving here.
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason) {
  ...
  for (const CTxIn &txin : it->GetTx().vin) mapNextTx.erase(txin.prevout);

  if (vTxHashes.size() > 1) {
    vTxHashes[it->vTxHashesIdx] = std::move(vTxHashes.back());
    vTxHashes[it->vTxHashesIdx].second->vTxHashesIdx = it->vTxHashesIdx;
    vTxHashes.pop_back();
    if (vTxHashes.size() * 2 < vTxHashes.capacity()) vTxHashes.shrink_to_fit();
  } else
    vTxHashes.clear();
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const {
  ...
  if (GetRand(std::numeric_limits<uint32_t>::max()) >= nCheckFrequency) return;
...
static TxMempoolInfo GetInfo(
    CTxMemPool::indexed_transaction_set::const_iterator it) {
...
bool CCoinsViewMemPool::GetCoin(const COutPoint &outpoint, Coin &coin) const {
  ...
  // guaranteed to never conflict with the underlying cache, and it cannot have
  // pruned entries (as it contains full) transactions. First checking the
  // underlying cache risks returning a pruned entry instead.
...
const CTxMemPool::setEntries &CTxMemPool::GetMemPoolParents(
    txiter entry) const {
...
const CTxMemPool::setEntries &CTxMemPool::GetMemPoolChildren(
    txiter entry) const {
  assert(entry != mapTx.end());
  txlinksMap::const_iterator it = mapLinks.find(entry);
  assert(it != mapLinks.end());
...
void CTxMemPool::TrimToSize(size_t sizelimit,
  ...
  while (!mapTx.empty() && DynamicMemoryUsage() > sizelimit) {
    ...
    // the "minimum reasonable fee rate" (ie some value under which we consider
    // txn to have 0 fee). This way, we don't allow txn to enter mempool with
    // feerate equal to txn which were removed with no block in between.
    ...
    if (pvNoSpendsRemaining) {
      ...
      for (txiter iter : stage) txn.push_back(iter->GetTx());
    }
