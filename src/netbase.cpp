 ...
 * requested or return False on error or timeout. This function can be
 * interrupted by calling InterruptSocks5()
 *
...
static IntrRecvError InterruptibleRecv(uint8_t* data, size_t len, int timeout,
  ...
  while (len > 0 && curTime < endTime) {
    ...
    if (interruptSocks5Recv) return IntrRecvError::Interrupted;
...
static bool Socks5(const std::string& strDest, int port,
  ...
  if ((recvr = InterruptibleRecv(pchRet1, 2, SOCKS5_RECV_TIMEOUT, hSocket)) !=
      IntrRecvError::OK) {
    LogPrintf(
        "Socks5() connect to %s:%d failed: InterruptibleRecv() timeout or "
        "other failure\n",
        strDest, port);
    return false;
  }
...
SOCKET CreateSocket(const CService& addrConnect) {
  ...
  SOCKET hSocket = socket(((struct sockaddr*)&sockaddr)->sa_family, SOCK_STREAM,
                          IPPROTO_TCP);
  ...
  if (!SetSocketNonBlocking(hSocket, true)) {
    CloseSocket(hSocket);
    LogPrintf(
        "ConnectSocketDirectly: Setting socket to non-blocking failed, error "
        "%s\n",
