...
class CBloomFilter {
 ...
 public:
   ...
   * with the given number of elements Note that if the given parameters will
   * result in a filter outside the bounds of the protocol limits, the filter
   * created will be as close to the given parameters as possible within the
   * protocol limits. This will apply if nFPRate is very low or nElements is
   * unreasonably high. nTweak is a constant which is added to the seed value
   * passed to the hash function It should generally always be a random value
   * (and is largely only exposed for unit testing) nFlags should be one of the
   * BLOOM_UPDATE_* enums (not _MASK)
   */
  CBloomFilter(const unsigned int nElements, const double nFPRate,
  ...
  //! functions is <= MAX_HASH_FUNCS (catch a filter which was just deserialized
  //! which was too big)
