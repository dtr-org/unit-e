 ...
 * experimental new digital currency called UnitE (https://www.unite.org/),
 * which enables instant payments to anyone, anywhere in the world. UnitE uses
...
bool AppInit(int argc, char* argv[]) {
  ...
  try {
    ...
    for (int i = 1; i < argc; i++) {
      if (!IsSwitchChar(argv[i][0])) {
        fprintf(stderr,
                ...
                "united -h for a list of options.\n",
                argv[i]);
        return false;
      }
    }

    // -server defaults to true for united but not for the GUI so do this here
    ...
    if (gArgs.GetBoolArg("-daemon", false)) {
      ...
      // Daemonize
#ifdef __APPLE__
      // daemon() is deprecated on macOS. glibc implements daemon() in terms of
      // fork, the same is being done here.
      switch (fork()) {
        case -1:
        ...
        case 0: {
          ...
          if (devnull) {
            // "close FDs" - redirect std in/out/err to/from /dev/null (like
            // daemon() would)
...
int main(int argc, char* argv[]) {
  ...
  // Connect united signal handlers
  noui_connect();
