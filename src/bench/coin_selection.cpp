...
static void CoinSelection(benchmark::State& state) {
  ...
  while (state.KeepRunning()) {
    // Empty wallet.
    for (COutput output : vCoins) delete output.tx;
    vCoins.clear();

    // Add coins.
    for (int i = 0; i < 1000; i++) addCoin(1000 * UNIT, wallet, vCoins);
    addCoin(3 * UNIT, wallet, vCoins);

    std::set<CInputCoin> setCoinsRet;
    CAmount nValueRet;
    bool success = wallet.SelectCoinsMinConf(1003 * UNIT, 1, 6, 0, vCoins,
