...
bool IsStringBoolPositive(const std::string &value) {
  return (value == "+" || value == "1" || icompare_str(value, "on") ||
          icompare_str(value, "true") || icompare_str(value, "yes") ||
          icompare_str(value, "y"));
}

bool IsStringBoolNegative(const std::string &value) {
  return (value == "-" || value == "0" || icompare_str(value, "off") ||
          icompare_str(value, "false") || icompare_str(value, "no") ||
          icompare_str(value, "n"));
}
...
std::string &LTrimWhitespace(std::string &s) {
  ...
  if (i != s.begin()) s.erase(s.begin(), i);
  return s;
}
...
std::string &RTrimWhitespace(std::string &s) {
  ...
  for (i = s.rbegin(); i != s.rend(); ++i)
    if (!std::isspace(*i)) break;
  if (i != s.rbegin()) s.erase(i.base(), s.end());
  return s;
}
