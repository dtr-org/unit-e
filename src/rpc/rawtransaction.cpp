...
UniValue verifytxoutproof(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "to, or empty array if the proof can not be validated.\n");
  ...
  if (mapBlockIndex[merkleBlock.header.GetHash()]->nTx ==
...
UniValue createrawtransaction(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "the unite address, the numeric value (can be string) is the " +
        CURRENCY_UNIT +
        " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "'pubkeyhash'\n"
        "         \"addresses\" : [           (json array of string)\n"
        "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) unite "
        "address\n"
        "           ,...\n"
        "         ]\n"
        "       }\n"
    ...
    for (unsigned int idx = 0; idx < prevTxs.size(); idx++) {
      ...
      if (fGivenKeys && (scriptPubKey.IsPayToScriptHash() ||
        ...
        if (!v.isNull()) {
          ...
          // Automatically also add the P2WSH wrapped version of the script (to
          // deal with P2SH-P2WSH).
  ...
  for (unsigned int i = 0; i < mtx.vin.size(); i++) {
    ...
    if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness,
      ...
      if (serror == SCRIPT_ERR_INVALID_STACK_OPERATION) {
        // Unable to sign input and verification failed (possible attempt to
        // partially sign).
        TxInErrorToJSON(
            txin, vErrors,
