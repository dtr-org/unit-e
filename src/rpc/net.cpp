...
UniValue getpeerinfo(const JSONRPCRequest& request) {
  ...
  for (const CNodeStats& stats : vstats) {
    ...
    if (stats.dPingTime > 0.0) obj.push_back(Pair("pingtime", stats.dPingTime));
    if (stats.dMinPing <
...
UniValue addnode(const JSONRPCRequest& request) {
  ...
  if (!request.params[1].isNull()) strCommand = request.params[1].get_str();
...
UniValue getaddednodeinfo(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "connected = true\n"
        "       {\n"
        "         \"address\" : \"192.168.0.201:7182\",  (string) The unite "
...
UniValue setban(const JSONRPCRequest& request) {
  ...
  if (strCommand == "add") {
    ...
    if (!request.params[2].isNull()) banTime = request.params[2].get_int64();

    bool absolute = false;
    if (request.params[3].isTrue()) absolute = true;
...
UniValue setnetworkactive(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() != 1) {
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"state\"        (boolean, required) true to enable networking, "
        "false to disable\n");
  }
