...
UniValue waitfornewblock(const JSONRPCRequest& request) {
  ...
  int timeout = 0;
  if (!request.params[0].isNull()) timeout = request.params[0].get_int();
...
UniValue waitforblock(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        HelpExampleCli("waitforblock",
                       "\"0000000000079f8ef3d2c688c244eb7a4570b24c9ed7b4a8c619e"
                       "b02596f8862\", 1000") +
  ...
  if (!request.params[1].isNull()) timeout = request.params[1].get_int();
...
UniValue getrawmempool(const JSONRPCRequest& request) {
  ...
  bool fVerbose = false;
  if (!request.params[0].isNull()) fVerbose = request.params[0].get_bool();
...
UniValue getmempoolancestors(const JSONRPCRequest& request) {
  ...
  bool fVerbose = false;
  if (!request.params[1].isNull()) fVerbose = request.params[1].get_bool();
...
UniValue getblockheader(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "  \"nTx\" : n,             (numeric) The number of transactions in "
        "the block.\n"
        ...
        HelpExampleCli("getblockheader",
                       "\"00000000c937983704a73af28acdec37b049d214adbda81d7e2a3"
                       "dd146f6ed09\"") +
...
UniValue pruneblockchain(const JSONRPCRequest& request) {
  ...
  if (chainHeight < Params().PruneAfterHeight())
    ...
    LogPrint(BCLog::RPC,
             "Attempt to prune blocks close to the tip.  Retaining the minimum "
             "number of blocks.");
...
UniValue gettxoutsetinfo(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "  \"bestblock\": \"hex\",   (string) The hash of the block at the tip "
        "of the chain\n"
        "  \"transactions\": n,      (numeric) The number of transactions with "
        "unspent outputs\n"
        "  \"txouts\": n,            (numeric) The number of unspent "
        "transaction outputs\n"
...
UniValue gettxout(const JSONRPCRequest& request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "\nResult:\n"
        "{\n"
        "  \"bestblock\":  \"hash\",    (string) The hash of the block at the "
        "tip of the chain\n"
        ...
        "     \"addresses\" : [          (array of string) array of unite "
        "addresses\n"
  ...
  bool fMempool = true;
  if (!request.params[2].isNull()) fMempool = request.params[2].get_bool();
...
UniValue verifychain(const JSONRPCRequest& request) {
  ...
  if (!request.params[0].isNull()) nCheckLevel = request.params[0].get_int();
  if (!request.params[1].isNull()) nCheckDepth = request.params[1].get_int();
...
UniValue getchaintips(const JSONRPCRequest& request) {
   ...
   * do not have another orphan building off of them. Algorithm:
