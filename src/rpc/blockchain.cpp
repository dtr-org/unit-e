...
UniValue readsnapshot(const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "\nExamples:\n"
                + HelpExampleCli("readsnapshot", "0")
                + HelpExampleRpc("readsnapshot", "0")
        );
    }
    ...
    int totalOutputs = 0;
    while (iter.Valid()) {
        snapshot::TxUTXOSet TxUTXOSet = iter.GetTxUTXOSet();
        totalOutputs += TxUTXOSet.m_outputs.size();
    ...
    rootNode.push_back(Pair("total_tx_utxo_sets", iter.GetTotalTxUTXOSets()));
...
UniValue createsnapshot(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            "createsnapshot (<maxTxUTXOSets>)\n"
            "\nCreates the snapshot of the UTXOs on the disk.\n"
            "\nCreates the snapshot of the UTXOs on the disk.\n"
            "\nArguments:\n"
            "1. maxtxutxosets (numeric, optional) Maximum transactions to dump into the file"
            "\nExamples:\n"
    ...
    if (!request.params.empty()) {
        creator.m_maxTxUTXOSets = static_cast<uint64_t>(request.params[0].get_int64());
    }
    ...
    if (info.m_status != +snapshot::Status::OK) {
        switch(info.m_status) {
            case snapshot::Status::WRITE_ERROR:
                ...
                break;
            case snapshot::Status::RESERVE_SNAPSHOT_ID_ERROR:
                rootNode.push_back(Pair("error", "can't reserve snapshot ID"));
                break;
            case snapshot::Status::SET_SNAPSHOT_ID_ERROR:
                rootNode.push_back(Pair("error", "can't set new snapshot ID"));
                break;
            case snapshot::Status::CALC_SNAPSHOT_HASH_ERROR:
    ...
    rootNode.push_back(Pair("total_utxo_sets", info.m_indexerMeta.m_totalTxUTXOSets));
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "blockchain",         "createsnapshot",         &createsnapshot,         {"maxtxutxosets"} },
