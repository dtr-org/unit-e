...
int64_t CChainParams::GetProofOfStakeReward(const CBlockIndex* pindexPrev,
                                            int64_t nFees) const {
  int64_t nSubsidy;
...
bool CChainParams::CheckImportCoinbase(int nHeight, uint256& hash) const {
  for (auto& cth : Params().vImportedCoinbaseTxns) {
    ...
    return error("%s - Hash mismatch at height %d: %s, expect %s.", __func__,
                 nHeight, hash.ToString(), cth.hash.ToString());
  };
 ...
 public:
  CMainParams() {
    ...
    consensus.nMinimumChainWork = uint256S(
        "0x000000000000000000000000000000000000000000f91c579d57cad4bc5278cc");

    // By default assume that the signatures in ancestors of this block are
    // valid.
    consensus.defaultAssumeValid =
        uint256S(
            "0x0000000000000000005214481d2d96f898e3d5416e43359c145944a909d242e"
            "0");  // 506067
    ...
    genesis =
        CreateGenesisBlock(1231006505, 2083236893, 0x1d00ffff, 1, 50 * UNIT);
    ...
    // fall back to using them as a oneshot if they dont support the service
    ...
    vSeeds.emplace_back("seed.unite.sipa.be");  // Pieter Wuille, only supports
                                                // x1, x5, x9, and xd
    vSeeds.emplace_back(
        "dnsseed.bluematt.me");  // Matt Corallo, only supports x9
    vSeeds.emplace_back("dnsseed.unite.dashjr.org");  // Luke Dashjr
    vSeeds.emplace_back(
        "seed.unitestats.com");  // Christian Decker, supports x1 - xf
    vSeeds.emplace_back(
        "seed.unite.jonasschnelli.ch");  // Jonas Schnelli, only supports x1,
                                         // x5, x9, and xd
    ...
    checkpointData = {{
        {11111, uint256S("0x0000000069e244f73d78e8fd29ba2fd2ed618bd6fa2ee92559f"
                         "542fdb26e7c1d")},
        {33333, uint256S("0x000000002dd5588a74784eaa7ab0507a18ad16a236e7b1ce69f"
                         "00d7ddfb5d0a6")},
        {74000, uint256S("0x0000000000573993a3c9e41ce34471c079dcf5f52a0e824a81e"
                         "7f953b8661a20")},
        {105000, uint256S("0x00000000000291ce28027faea320c8d2b054b2e0fe44a773f3"
                          "eefb151d6bdc97")},
        {134444, uint256S("0x00000000000005b12ffd4cd315cd34ffd4a594f430ac814c91"
                          "184a0d42d2b0fe")},
        {168000, uint256S("0x000000000000099e61ea72015e79632f216fe6cb33d7899acb"
                          "35b75c8303b763")},
        {193000, uint256S("0x000000000000059f452a5f7340de6682a977387c17010ff6e6"
                          "c3bd83ca8b1317")},
        {210000, uint256S("0x000000000000048b95347e83192f69cf0366076336c639f9b7"
                          "228e9ba171342e")},
        {216116, uint256S("0x00000000000001b4f4b433e81ee46494af945cf96014816a4e"
                          "2370f11b23df4e")},
        {225430, uint256S("0x00000000000001c108384350f74090433e7fcf79a606b8e797"
                          "f065b130575932")},
        {250000, uint256S("0x000000000000003887df1f29024b06fc2200b55f8af8f35453"
                          "d7be294df2d214")},
        {279000, uint256S("0x0000000000000001ae8c72a0b0c301f67e3afca10e819efa90"
                          "41e458e9bd7e40")},
        {295000, uint256S("0x00000000000000004d9b4ef50f0f9d686fd69db2e03af35a10"
                          "0370c64632a983")},
    }};

    chainTxData = ChainTxData{
        // Data as of block
        // 0000000000000000002d6cca6761c99b3c2e936f9a0e304b7c7651a993f461de
        // (height 506081).
        1516903077,  // * UNIX timestamp of last known number of transactions
        295363220,   // * total number of transactions between genesis and that
                     // timestamp
        //   (the tx=... number in the SetBestChain debug.log lines)
        3.5  // * estimated number of transactions per second after that
             // timestamp
    };
  }
};
 ...
 public:
  CTestNetParams() {
    ...
    consensus.BIP16Height =
        514;  // 00000000040b4e986385315e14bee30ad876d8b47f748025b26683116d21aa65
    ...
    consensus.defaultAssumeValid =
        uint256S(
            "0x0000000002e9e7b00e1f6dc5123a04aad68dd0f0968d8c7aa45f6640795c37b"
            "1");  // 1135275
    ...
    genesis =
        CreateGenesisBlock(1296688602, 414098458, 0x1d00ffff, 1, 50 * UNIT);
    consensus.hashGenesisBlock = genesis.GetHash();
    assert(consensus.hashGenesisBlock ==
    ...
    chainTxData = ChainTxData{// Data as of block
                              // 000000000000033cfa3c975eb83ecf2bb4aaedf68e6d279f6ed2b427c64caff9
                              // (height 1260526)
                              1516903490, 17082348, 0.09};
  }
};
 ...
 public:
  CRegTestParams() {
    ...
    consensus.BIP34Height =
        100000000;  // BIP34 has not activated on regtest (far in the future so
                    // block v1 are not rejected in tests)
