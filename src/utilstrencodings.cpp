...
bool DecodeBase16(const std::string& input, std::vector<uint8_t>& output) {
  ...
  for (size_t i = 0; i < len; i += 2) {
    char a = input[i];
    const char* p =
        std::lower_bound(BASE_16_ALPHABET, BASE_16_ALPHABET + 16, a);
    if (*p != a) {
      return false;
    }
    char b = input[i + 1];
    const char* q =
        std::lower_bound(BASE_16_ALPHABET, BASE_16_ALPHABET + 16, b);
    if (*q != b) {
      return false;
    }
    output.push_back((static_cast<uint8_t>(p - BASE_16_ALPHABET) << 4) |
                     static_cast<uint8_t>(q - BASE_16_ALPHABET));
  }
  return true;
}
