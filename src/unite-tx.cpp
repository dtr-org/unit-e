...
static int AppInitRawTx(int argc, char* argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    std::string strUsage =
        strprintf(_("%s unite-tx utility version"), _(PACKAGE_NAME)) + " " +
        FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
        "  unite-tx [options] <hex-tx> [commands]  " +
        _("Update hex-encoded unite transaction") + "\n" +
        "  unite-tx [options] -create [commands]   " +
        _("Create hex-encoded unite transaction") + "\n" + "\n";
...
static void MutateTxAddInput(CMutableTransaction& tx,
  ...
  if (vStrInputParts.size() > 2) nSequenceIn = std::stoul(vStrInputParts[2]);
...
static void MutateTxAddOutData(CMutableTransaction& tx,
  ...
  if (pos == 0) throw std::runtime_error("TX output value not specified");
  ...
  if (!IsHex(strData)) throw std::runtime_error("invalid TX output data");
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr) {
  ...
  for (unsigned int kidx = 0; kidx < keysObj.size(); kidx++) {
    ...
    if (!fGood) throw std::runtime_error("privatekey not valid");
  ...
  UniValue prevtxsObj = registers["prevtxs"];
  {
    for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
      ...
      if (nOut < 0) throw std::runtime_error("vout must be positive");
...
static void OutputTxHash(const CTransaction& tx) {
  std::string strHexHash =
      tx.GetHash().GetHex();  // the hex-encoded transaction hash (aka the
                              // transaction id)
...
static std::string readStdin() {
  ...
  if (ferror(stdin)) throw std::runtime_error("error reading stdin");
...
static int CommandLineRawTx(int argc, char* argv[]) {
  ...
  try {
    ...
    if (!fCreateBlank) {
      ...
      if (argc < 2) throw std::runtime_error("too few parameters");
