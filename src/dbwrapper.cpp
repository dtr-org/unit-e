...
class CUnitELevelDBLogger : public leveldb::Logger {
 public:
  ...
  // vsprintf. Please do not do this in normal code
...
static leveldb::Options GetOptions(size_t nCacheSize) {
  ...
  if (leveldb::kMajorVersion > 1 ||
    ...
    // trigger error on corruption in later versions.
...
void HandleError(const leveldb::Status& status) {
  if (status.ok()) return;
  LogPrintf("%s\n", status.ToString());
  if (status.IsCorruption()) throw dbwrapper_error("Database corrupted");
  if (status.IsIOError()) throw dbwrapper_error("Database I/O error");
  if (status.IsNotFound()) throw dbwrapper_error("Database entry missing");
