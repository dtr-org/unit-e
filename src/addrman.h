 ...
 * selection chances for addresses that are seen frequently. The chance for
 * increasing this multiplicity decreases exponentially.
 *      * When adding a new address to a full bucket, a randomly chosen entry
 * (with a bias favoring less recently seen ones) is removed from it first.
 ...
 * entry (with a bias favoring less recently tried ones) is evicted from it,
 * back to the "new" buckets.
 ...
 * will introduce frequent (and expensive) consistency checks for the entire
 * data structure.
 */
...
class CAddrMan {
 ...
 public:
   ...
   * detect it as incompatible. This is necessary because it did not check the
   * version number on deserialization.
   *
  ...
  void Unserialize(Stream &s) {
    ...
    for (int n = 0; n < nNew; n++) {
      ...
      if (nVersion != 1 || nUBuckets != ADDRMAN_NEW_BUCKET_COUNT) {
        ...
        // bucket count wrong), immediately try to give them a reference based
        // on their primary source address.
