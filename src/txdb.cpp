...
struct CoinEntry {
  ...
  explicit CoinEntry(const COutPoint *ptr)
      : outpoint(const_cast<COutPoint *>(ptr)), key(DB_UNIT) {}
...
bool CCoinsViewDBCursor::Valid() const { return keyTmp.first == DB_UNIT; }

void CCoinsViewDBCursor::Next() {
  pcursor->Next();
  CoinEntry entry(&keyTmp.second);
  if (!pcursor->Valid() || !pcursor->GetKey(entry)) {
    keyTmp.first = 0;  // Invalidate cached key after last record so that
                       // Valid() and GetKey() return false
  } else {
