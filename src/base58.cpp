...
void CBase58Data::SetData(const std::vector<unsigned char>& vchVersionIn,
  ...
  if (!vchData.empty()) memcpy(vchData.data(), pdata, nSize);
}
...
CTxDestination DecodeDestination(const std::string& str,
  ...
  if (bech.second.size() > 0 && bech.first == params.Bech32HRP()) {
    // Bech32 decoding
    int version =
        bech.second[0];  // The first 5 bit symbol is the witness version (0-16)
...
bool CUnitESecret::SetString(const char* pszSecret) {
  return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CUnitESecret::SetString(const std::string& strSecret) {
