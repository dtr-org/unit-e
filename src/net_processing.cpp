...
void ProcessBlockAvailability(NodeId nodeid) {
    ...
    if (!state->hashLastUnknownBlock.IsNull()) {
        ...
        if (itOld != mapBlockIndex.end() && itOld->second->chain_stake > 0) {
            if (state->pindexBestKnownBlock == nullptr || itOld->second->chain_stake >= state->pindexBestKnownBlock->chain_stake)
...
void UpdateBlockAvailability(NodeId nodeid, const uint256 &hash) {
    ...
    if (it != mapBlockIndex.end() && it->second->chain_stake > 0) {
        // An actually better block was announced.
        if (state->pindexBestKnownBlock == nullptr || it->second->chain_stake >= state->pindexBestKnownBlock->chain_stake)
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) {
    ...
    if (state->pindexBestKnownBlock == nullptr || state->pindexBestKnownBlock->chain_stake < chainActive.Tip()->chain_stake || state->pindexBestKnownBlock->chain_stake < nMinimumChainWork) {
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
...

    {
        ...
        if (received_new_header && pindexLast->chain_stake > chainActive.Tip()->chain_stake) {
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->chain_stake <= pindexLast->chain_stake) {
            std::vector<const CBlockIndex*> vToFetch;
        ...
        if (IsInitialBlockDownload() && nCount != MAX_HEADERS_RESULTS) {
            ...
            if (nodestate->pindexBestKnownBlock && nodestate->pindexBestKnownBlock->chain_stake < nMinimumChainWork) {
        ...
        if (!pfrom->fDisconnect && IsOutboundDisconnectionCandidate(pfrom) && nodestate->pindexBestKnownBlock != nullptr) {
            ...
            if (g_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->chain_stake >= chainActive.Tip()->chain_stake && !nodestate->m_chain_sync.m_protect) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (received_new_header && pindex->chain_stake > chainActive.Tip()->chain_stake) {
            nodestate->m_last_block_announcement = GetTime();
        }
        ...
        if (pindex->chain_stake <= chainActive.Tip()->chain_stake || // We know something better
...
void PeerLogicValidation::ConsiderEviction(CNode *pto, int64_t time_in_seconds)
{
    ...
    if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->chain_stake >= chainActive.Tip()->chain_stake) {
            if (state.m_chain_sync.m_timeout != 0) {
                state.m_chain_sync.m_timeout = 0;
                state.m_chain_sync.m_work_header = nullptr;
                state.m_chain_sync.m_sent_getheaders = false;
            }
        } else if (state.m_chain_sync.m_timeout == 0 || (state.m_chain_sync.m_work_header != nullptr && state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->chain_stake >= state.m_chain_sync.m_work_header->chain_stake)) {
