...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count,
  ...
  while (pindexWalk->nHeight < nMaxHeight) {
    ...
    // mean time, update pindexLastCommonBlock as long as all ancestors are
...
void PeerLogicValidation::BlockChecked(const CBlock& block,
  ...
  if (state.IsInvalid(nDoS)) {
    ...
    if (it != mapBlockSource.end() && State(it->second.first) &&
      ...
      if (nDoS > 0 && it->second.second) Misbehaving(it->second.first, nDoS);
    }
  }
  // Check that:
...
void static ProcessGetBlockData(CNode* pfrom,
  ...
  // historical blocks never disconnect whitelisted nodes
...
bool static ProcessHeadersMessage(CNode* pfrom, CConnman* connman,
...

  {
    ...
    if (nCount == MAX_HEADERS_RESULTS) {
      ...
      // pindexBestHeader, continue from there instead.
      LogPrint(BCLog::NET,
    ...
    if (IsInitialBlockDownload() && nCount != MAX_HEADERS_RESULTS) {
      ...
      if (nodestate->pindexBestKnownBlock &&
        ...
        if (IsOutboundDisconnectionCandidate(pfrom)) {
          LogPrintf(
              "Disconnecting outbound peer %d -- headers chain has "
              "insufficient work\n",
              pfrom->GetId());
          pfrom->fDisconnect = true;
        }
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand,
  ...
  if (strCommand == NetMsgType::REJECT) {
    ...
    if (nServices & ((1 << 7) | (1 << 5))) {
      if (GetTime() < 1533096000) {
        ...
        // August 1st, 2018 These bits have been used as a flag to indicate that
        // a node is running incompatible consensus rules instead of changing
        // the network magic, so we're stuck disconnecting based on these
        // service bits, at least for a while.
        pfrom->fDisconnect = true;
        return false;
      }
    }
  ...
  if (strCommand == NetMsgType::VERACK) {
    ...
    if (pfrom->nVersion >= SHORT_IDS_BLOCKS_VERSION) {
      ...
      if (pfrom->GetLocalServices() & NODE_WITNESS)
        connman->PushMessage(
            pfrom, msgMaker.Make(NetMsgType::SENDCMPCT,
                                 fAnnounceUsingCMPCTBLOCK, nCMPCTBLOCKVersion));
      nCMPCTBLOCKVersion = 1;
      connman->PushMessage(
          pfrom, msgMaker.Make(NetMsgType::SENDCMPCT, fAnnounceUsingCMPCTBLOCK,
                               nCMPCTBLOCKVersion));
    }
    ...
    for (CAddress& addr : vAddr) {
      ...
      if (!MayHaveUsefulAddressDB(addr.nServices)) continue;
    ...
    for (CInv& inv : vInv) {
      ...
      if (inv.type == MSG_BLOCK) {
        ...
        if (!fAlreadyHave && !fImporting && !fReindex &&
          ...
          // headers-announcements are now the primary method of announcement on
          // the network, and since, in the case that a node fell back to inv we
          // probably have a reorg which we should get the headers for first, we
          // now only provide a getheaders response here. When we receive the
          // headers, we will then ask for the blocks we need.
          connman->PushMessage(
              pfrom, msgMaker.Make(NetMsgType::GETHEADERS,
    ...
    if (it->second->nHeight < chainActive.Height() - MAX_BLOCKTXN_DEPTH) {
      ...
      // we'll respond then (without cs_main)
      return true;
    }
    ...
    if (!AlreadyHave(inv) &&
      ...
      while (!vWorkQueue.empty()) {
        ...
        for (auto mi = itByPrev->second.begin(); mi != itByPrev->second.end();
          ...
          // counter-DoS based on orphan resolution (that is, feeding people an
          // invalid transaction based on LegitTxX in order to get anyone
      ...
      if (pfrom->fWhitelisted &&
        ...
        if (!state.IsInvalid(nDoS) || nDoS == 0) {
          ...
          LogPrintf(
              "Not relaying invalid transaction %s from whitelisted peer=%d "
              "(%s)\n",
              tx.GetHash().ToString(), pfrom->GetId(),
    ...
    if (state.IsInvalid(nDoS)) {
      ...
      if (state.GetRejectCode() > 0 &&
          state.GetRejectCode() <
              REJECT_INTERNAL)  // Never send AcceptToMemoryPool's internal
                                // codes over P2P
        connman->PushMessage(pfrom,
      ...
      if (pindex->nHeight <= chainActive.Height() + 2) {
        if ((!fAlreadyInFlight &&
          ...
          if (status == READ_STATUS_INVALID) {
            MarkBlockAsReceived(
                pindex->GetBlockHash());  // Reset in-flight state in case of
                                          // whitelist
            Misbehaving(pfrom->GetId(), 100);
           ...
           !fReindex)  // Ignore blocks received while importing
  {
    ...
    bool fBlockRead = false;
    {
      ...
      if (status == READ_STATUS_INVALID) {
        ...
        Misbehaving(pfrom->GetId(), 100);
        LogPrintf(
            "Peer %d sent us invalid compact block/non-matching block "
            "transactions\n",
            pfrom->GetId());
        return true;
      } else if (status == READ_STATUS_FAILED) {
        ...
        // scores, so the race between here and cs_main in ProcessNewBlock is
        // fine. BIP 152 permits peers to relay compact blocks after validating
    ...
    if (fBlockRead) {
      ...
      // to be processed, even if it would not be a candidate for new tip
      // (missing previous block, chain not long enough, etc) This bypasses some
      // anti-DoS logic in AcceptBlock (eg to prevent disk-space attacks), but
      // this should be safe due to the protections in the compact block handler
      // -- see related comment in compact block optimistic reconstruction
      // handling.
      ProcessNewBlock(chainparams, pblock, /*fForceProcessing=*/true,
                      &fNewBlock);
      if (fNewBlock) {
           ...
           !fReindex)  // Ignore blocks received while importing
  {
    ...
    // data object, and thus, the maximum size any matched object can have) in a
    // filteradd message
    bool bad = false;
...
void PeerLogicValidation::ConsiderEviction(CNode* pto,
  ...
  if (!state.m_chain_sync.m_protect && IsOutboundDisconnectionCandidate(pto) &&
    ...
    if (state.pindexBestKnownBlock != nullptr &&
      ...
      // noticing that for the first time, OR this peer was able to catch up to
      // some earlier point where we checked against our tip. Either way, set a
      // new timeout based on current tip.
      ...
      if (state.m_chain_sync.m_sent_getheaders) {
        ...
        LogPrintf(
            "Disconnecting outbound peer %d for old chain, best known block = "
            "%s\n",
            pto->GetId(),
            state.pindexBestKnownBlock != nullptr
...
bool PeerLogicValidation::SendMessages(CNode* pto,
  ...
  const Consensus::Params& consensusParams = Params().GetConsensus();
  {
    ...
    TRY_LOCK(cs_main, lockMain);  // Acquire cs_main for
                                  // IsInitialBlockDownload() and CNodeState()
    ...
    if (pto->nNextAddrSend < nNow) {
      ...
      if (pto->vAddrToSend.capacity() > 40) pto->vAddrToSend.shrink_to_fit();
    }
    ...
    bool fFetch =
        ...
        (nPreferredDownload == 0 && !pto->fClient &&
         !pto->fOneShot);  // Download if this is a nice peer, or we have no
      ...
      if (pindexBestHeader->GetBlockTime() <=
        ...
        if (nNow > state.nHeadersSyncTimeout && nSyncStarted == 1 &&
          ...
          if (!pto->fWhitelisted) {
            ...
            LogPrintf(
                "Timeout downloading headers from whitelisted peer=%d, not "
                "disconnecting\n",
                pto->GetId());
