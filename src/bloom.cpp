...
CBloomFilter::CBloomFilter(const unsigned int nElements, const double nFPRate,
    ...
    : /**
       ...
       * elements Again, we ignore filter parameters which will create a bloom
       * filter with more hash functions than the protocol limits See
       * https://en.wikipedia.org/wiki/Bloom_filter for an explanation of these
       * formulas
       */
      isFull(false),
      isEmpty(true),
...
inline unsigned int CBloomFilter::Hash(
...
bool CBloomFilter::IsRelevantAndUpdate(const CTransaction& tx) {
  ...
  for (unsigned int i = 0; i < tx.vout.size(); i++) {
    ...
    // scriptPubKey in tx If this matches, also add the specific output that was
    // matched. This means clients don't have to update the filter themselves
    // when a new relevant tx is discovered in order to find spending
    // transactions, which avoids round-tripping and race conditions.
...
static inline uint32_t RollingBloomHash(
