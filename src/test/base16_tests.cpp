...
std::vector<uint8_t> toUtf8Vector(const char(&text)[length])
{
    ...
    for (std::size_t i = 0; i < length - 1; ++i) { // exclude null terminating byte
...
BOOST_AUTO_TEST_CASE(encode_base16)
{
    ...
    BOOST_CHECK_EQUAL(output, "ec9588eb8595ed9598ec84b8ec9a942c20eb8bb9ec8ba0ec9d8020ec96b4eb96a0ec84b8ec9a943f");
}

BOOST_AUTO_TEST_CASE(decode_base16)
{
    const std::string input = "ec9588eb8595ed9598ec84b8ec9a942c20eb8bb9ec8ba0ec9d8020ec96b4eb96a0ec84b8ec9a943f";
    std::vector<uint8_t> output;
    const bool result = DecodeBase16(input, output);
    BOOST_CHECK_EQUAL(result, true);
    const std::vector<uint8_t> expectedOutput = toUtf8Vector(u8"안녕하세요, 당신은 어떠세요?");
