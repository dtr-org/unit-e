...
class VersionBitsTester {
  ...
  // etc... This is to test whether lack of cached information leads to the same
  // results.
  ...
  VersionBitsTester& TestFailed() {
    for (int i = 0; i < CHECKERS; i++) {
      if (InsecureRandBits(i) == 0) {
        BOOST_CHECK_MESSAGE(
            checker[i].GetStateFor(
                vpblock.empty() ? nullptr : vpblock.back()) == THRESHOLD_FAILED,
...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion) {
  ...
  // so CBV should still not yet set the bit.
  nTime = nStartTime;
  ...
  BOOST_CHECK_EQUAL(
      ComputeBlockVersion(lastBlock, mainnetParams) & VERSIONBITS_TOP_MASK,
  ...
  // next period.
  lastBlock =
      secondChain.Mine(2016, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
  BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1 << bit)) !=
              0);

  // Mine another period worth of blocks, signaling the new bit.
  lastBlock =
      secondChain.Mine(4032, nTime, VERSIONBITS_TOP_BITS | (1 << bit)).Tip();
  ...
  lastBlock =
      secondChain.Mine(6047, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
  BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1 << bit)) !=
              0);
  lastBlock =
      secondChain.Mine(6048, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
  BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1 << bit),
                    0);
