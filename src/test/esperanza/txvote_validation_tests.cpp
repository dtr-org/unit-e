...
CTransaction CreateVote(esperanza::VoteData& vote,
...
BOOST_FIXTURE_TEST_CASE(tx_mempool_accept_vote, TestChain100Setup) {
  ...
  esperanza::VoteData voteData{depositTx.GetHash(), GetRandHash(), 0, 10};
  CTransaction voteTx = CreateVote(voteData, depositTx, coinbaseKey);
