...
BOOST_AUTO_TEST_CASE(getlocator_test) {
  ...
  for (unsigned int i = 0; i < vBlocksMain.size(); i++) {
    vHashMain[i] = ArithToUint256(i);  // Set the hash equal to the height, so
                                       // we can quickly check the distances.
  ...
  for (unsigned int i = 0; i < vBlocksSide.size(); i++) {
    vHashSide[i] = ArithToUint256(
        i + 50000 +
        (arith_uint256(1) << 128));  // Add 1<<128 to the hashes, so GetLow64()
                                     // still returns the height.
