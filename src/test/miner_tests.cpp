...
void TestPackageSelection(const CChainParams& chainparams, CScript scriptPubKey,
  ...
  tx.vout[1].nValue = 100000000;  // 1UNT output
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity) {
  ...
  CScript scriptPubKey = CScript()
                         << ParseHex(
                                ...
                                "09a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51e"
                                "c112de5c384df7ba0b8d578a4c702b6bf11d5f")
                         << OP_CHECKSIG;
  std::unique_ptr<CBlockTemplate> pblocktemplate;
  ...
  for (unsigned int i = 0; i < sizeof(blockinfo) / sizeof(*blockinfo); ++i) {
    CBlock* pblock = &pblocktemplate->block;  // pointer for convenience
    {
      ...
      txCoinbase.vout.resize(1);  // Ignore the (optional) segwit commitment
                                  // added by CreateNewBlock (as the hardcoded
                                  // nonces don't account for this)
