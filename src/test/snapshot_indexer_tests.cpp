...
BOOST_AUTO_TEST_CASE(snapshot_indexer_flush) {
  ...
  for (uint64_t i = 0; i < totalMsgs; ++i) {
    ...
    s >> utxoSet.m_txId;
    streamIn << utxoSet;
    BOOST_CHECK(idx->WriteTxUTXOSet(utxoSet));
  }
  BOOST_CHECK(idx->Flush());
...
BOOST_AUTO_TEST_CASE(snapshot_indexer_writer) {
  ...
  for (uint64_t i = 0; i < totalMsgs; ++i) {
    snapshot::TxUTXOSet utxoSet;
    stream << utxoSet;
    BOOST_CHECK(indexer.WriteTxUTXOSet(utxoSet));
    BOOST_CHECK_EQUAL(indexer.GetMeta().m_totalTxUTXOSets, i + 1);
  }
...
BOOST_AUTO_TEST_CASE(snapshot_indexer_open) {
  ...
  uint64_t totalMsgs = (step * stepsPerFile) * 2 + step;
  for (uint64_t i = 0; i < totalMsgs; ++i) {
    BOOST_CHECK(indexer.WriteTxUTXOSet(snapshot::TxUTXOSet()));
    BOOST_CHECK_EQUAL(indexer.GetMeta().m_totalTxUTXOSets, i + 1);
  }
  BOOST_CHECK(indexer.Flush());
  ...
  BOOST_CHECK_EQUAL(openedIdx->GetMeta().m_totalTxUTXOSets, totalMsgs);
}
