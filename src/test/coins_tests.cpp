...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test) {
  ...
  for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
    // Do a random modification.
    {
      ...
      // As these functions can influence each other's behaviour by pulling
      // things into the cache, all combinations are tested.
...
// has the expected effect (the other duplicate is overwritten at all cache
// levels)
  ...
  for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
    ...
    if (randiter % 20 < 19) {
      ...
      // 17/20 times reconnect previous or add a regular tx
        ...
        // work properly if that ever happens and not resurrect the previously
        // overwritten coinbase
