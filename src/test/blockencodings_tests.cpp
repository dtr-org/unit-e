...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest) {
  ...
  // Do a simple ShortTxIDs RT
  {
    ...
    {
      PartiallyDownloadedBlock tmp = partialBlock;
      partialBlock.FillBlock(
          block2, {block.vtx[2]});  // Current implementation doesn't check txn
                                    // here, but don't require that
      partialBlock = tmp;
    }
    bool mutated;
...
BOOST_AUTO_TEST_CASE(NonCoinbasePreforwardRTTest) {
  ...
  // Test with pre-forwarding tx 1, but not coinbase
  {
    ...
    // Wrong transaction
    {
      PartiallyDownloadedBlock tmp = partialBlock;
      partialBlock.FillBlock(
          block2, {block.vtx[1]});  // Current implementation doesn't check txn
                                    // here, but don't require that
      partialBlock = tmp;
    }
    bool mutated;
    ...
    block3.vtx.clear();
    BOOST_CHECK_EQUAL(
        pool.mapTx.find(txhash)->GetSharedTx().use_count(),
