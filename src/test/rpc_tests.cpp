...
BOOST_AUTO_TEST_CASE(rpc_format_monetary_values) {
  ...
  BOOST_CHECK_EQUAL(ValueFromAmount((UNIT / 10000) * 123456789).write(),
                    "12345.67890000");
  BOOST_CHECK_EQUAL(ValueFromAmount(-UNIT).write(), "-1.00000000");
  BOOST_CHECK_EQUAL(ValueFromAmount(-UNIT / 10).write(), "-0.10000000");

  BOOST_CHECK_EQUAL(ValueFromAmount(UNIT * 100000000).write(),
                    "100000000.00000000");
  BOOST_CHECK_EQUAL(ValueFromAmount(UNIT * 10000000).write(),
                    "10000000.00000000");
  BOOST_CHECK_EQUAL(ValueFromAmount(UNIT * 1000000).write(),
                    "1000000.00000000");
...
BOOST_AUTO_TEST_CASE(json_parse_errors) {
  ...
  BOOST_CHECK_THROW(ParseNonRFCJSONValue("1.0]"), std::runtime_error);
  // UNT addresses should fail parsing
...
BOOST_AUTO_TEST_CASE(rpc_ban) {
  ...
  BOOST_CHECK_EQUAL(banned_until.get_int64(),
                    9907731200);  // absolute time check
