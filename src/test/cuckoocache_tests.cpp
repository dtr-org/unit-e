...
void insecure_GetRandHash(uint256& t) {
  ...
  for (uint8_t j = 0; j < 8; ++j) *(ptr++) = local_rand_ctx.rand32();
}
...
double test_cache(size_t megabytes, double load) {
  ...
  /** Do the insert */
  for (uint256& h : hashes_insert_copy) set.insert(h);
  /** Count the hits */
  uint32_t count = 0;
  for (uint256& h : hashes) count += set.contains(h, false);
...
void test_cache_erase(size_t megabytes) {
  ...
  for (uint32_t i = 0; i < (n_insert / 4); ++i) set.contains(hashes[i], true);
...
void test_cache_erase_parallel(size_t megabytes) {
  ...
  for (uint32_t x = 0; x < 3; ++x)
    ...
    threads.emplace_back([&, x] {
      ...
      for (uint32_t i = start; i < end; ++i) set.contains(hashes[i], true);
    });
...
void test_cache_generations() {
  ...
  // == 99.89% hit rate with low variance.
