...
BOOST_AUTO_TEST_CASE(snapshot_iterator) {
...

  {
    ...
    BOOST_CHECK_EQUAL(iter.GetTotalTxUTXOSets(), msgsToGenerate);

    // iterate sequentially
    uint32_t count = 0;
    while (iter.Valid()) {
      uint32_t value = 1000 + count;
      BOOST_CHECK_EQUAL(iter.GetTxUTXOSet().m_outputs.at(count).nValue, value);
      iter.Next();
      ++count;
    }
    ...
    for (uint32_t i = 0; i < msgsToGenerate; ++i) {
      ...
      int value = 1000 + i;
      BOOST_CHECK_EQUAL(iter.GetTxUTXOSet().m_outputs.at(i).nValue, value);
    }
    ...
    for (uint32_t i = msgsToGenerate; i > 0; --i) {
      ...
      BOOST_CHECK_EQUAL(iter.GetTxUTXOSet().m_outputs.at(i - 1).nValue, value);
    }
  }
}
