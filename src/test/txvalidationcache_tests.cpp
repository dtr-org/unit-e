...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    CScript scriptPubKey = CScript() <<  ToByteVector(coinstakeKey.GetPubKey()) << OP_CHECKSIG;

    // Create a double-spend of mature coinstake txn:
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vin[0].prevout.hash = coinstakeTxns[0].GetHash();
        ...
        BOOST_CHECK(coinstakeKey.Sign(hash, vchSig));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    CScript p2pk_scriptPubKey = CScript() << ToByteVector(coinstakeKey.GetPubKey()) << OP_CHECKSIG;
    CScript p2sh_scriptPubKey = GetScriptForDestination(CScriptID(p2pk_scriptPubKey));
    CScript p2pkh_scriptPubKey = GetScriptForDestination(coinstakeKey.GetPubKey().GetID());
    CScript p2wpkh_scriptPubKey = GetScriptForWitness(p2pkh_scriptPubKey);

    CBasicKeyStore keystore;
    keystore.AddKey(coinstakeKey);
    ...
    spend_tx.vin[0].prevout.hash = coinstakeTxns[0].GetHash();
    ...
    spend_tx.vout[2].scriptPubKey = CScript() << OP_CHECKLOCKTIMEVERIFY << OP_DROP << ToByteVector(coinstakeKey.GetPubKey()) << OP_CHECKSIG;
    spend_tx.vout[3].nValue = 11*EEES;
    spend_tx.vout[3].scriptPubKey = CScript() << OP_CHECKSEQUENCEVERIFY << OP_DROP << ToByteVector(coinstakeKey.GetPubKey()) << OP_CHECKSIG;
    ...
    // for the same tx with a different witness.
    {
        ...
        ProduceSignature(MutableTransactionSignatureCreator(&keystore, &valid_with_witness_tx, 0, 11*EEES, SIGHASH_ALL), spend_tx.vout[1].scriptPubKey, sigdata);
...

    {
        ...
        for (int i=0; i<2; ++i) {
            SignatureData sigdata;
            ProduceSignature(MutableTransactionSignatureCreator(&keystore, &tx, i, 11*EEES, SIGHASH_ALL), spend_tx.vout[i].scriptPubKey, sigdata);
