...
enum BlockStatus : uint32_t {
   ...
   * transactions valid, no duplicate txids, sigops, size, merkle root. Implies
   * all parents are at least TREE but not necessarily TRANSACTIONS. When all
 ...
 * pointing to it, but at most one of them can be part of the currently active
 * branch.
 */
class CBlockIndex {
 public:
  ...
  //! this block and all its parents are available. Change to 64-bit type when
  ...
  //! Proof-of-Stake: the stake modifier is a hash of some entropy bits to make
  //! pre-computing blocks more difficult
  uint256 bnStakeModifier;

  //! Proof-of-Stake: the previous stake (used for kernel hash and stake
  //! modifier)
  COutPoint prevoutStake;

  //! Proof-of-Stake: total money supply in the chain up to this point (used to
...
class CDiskBlockIndex : public CBlockIndex {
 public:
  ...
  inline void SerializationOp(Stream& s, Operation ser_action) {
    ...
    if (!(s.GetType() & SER_GETHASH)) READWRITE(VARINT(_nVersion));
