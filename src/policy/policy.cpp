...
bool AreInputsStandard(const CTransaction& tx,
  ...
  if (tx.IsCoinBase()) return true;  // Coinbases don't use vin normally
...
bool IsWitnessStandard(const CTransaction& tx,
                       const CCoinsViewCache& mapInputs) {
  if (tx.IsCoinBase()) return true;  // Coinbases are skipped

  for (unsigned int i = 0; i < tx.vin.size(); i++) {
    // We don't care if witness for this input is empty, since it must not be
    // bloated. If the script is invalid without witness, it would be caught
    // sooner or later during validation.
    if (tx.vin[i].scriptWitness.IsNull()) continue;
    ...
    if (prevScript.IsPayToScriptHash()) {
      ...
      // casually by converting the scriptSig into a stack. We do not check
      // IsPushOnly nor compare the hash as these will be done later anyway. If
      // the check fails at this stage, we know that this txid must be a bad
      // one.
      if (!EvalScript(stack, tx.vin[i].scriptSig, SCRIPT_VERIFY_NONE,
                      BaseSignatureChecker(), SIGVERSION_BASE))
        return false;
    ...
    if (witnessversion == 0 && witnessprogram.size() == 32) {
      ...
      if (sizeWitnessStack > MAX_STANDARD_P2WSH_STACK_ITEMS) return false;
