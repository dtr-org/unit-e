...
class TxConfirmStats {
 private:
  ...
  const std::map<double, unsigned int>&
      bucketMap;  // Map of bucket upper-bound to index into all vectors by
                  // bucket
  ...
  // each Y,X Combine the total value with the tx counts to calculate the avg
  // feerate per bucket
 ...
 public:
   ...
   * pass minSuccess OR return the highest feerate such that all lower values
   * fail minSuccess
...
double TxConfirmStats::EstimateMedianVal(int confTarget, double sufficientTxVal,
  ...
  // look at successively smaller buckets until we reach failure.  Otherwise, we
  // are looking for the highest feerate such that all lower values fail, and we
  ...
  // conditions Find the bucket with the median transaction and then report the
  // average feerate from that bucket This is a compromise between finding the
  // median which we can't since we don't save all tx's and reporting the
  // average which is less accurate
...
void TxConfirmStats::Read(CAutoFile& filein, int nFileVersion,
  ...
  if (decay <= 0 || decay >= 1) {
    throw std::runtime_error(
        "Corrupt estimates file. Decay must be between 0 and 1 "
        "(non-inclusive)");
  }
  filein >> scale;
  if (scale == 0) {
  ...
  if (maxConfirms <= 0 || maxConfirms > 6 * 24 * 7) {  // one week
    throw std::runtime_error(
        "Corrupt estimates file.  Must maintain estimates for between 1 and "
        "1008 (one week) confirms");
  }
  for (unsigned int i = 0; i < maxPeriods; i++) {
    if (confAvg[i].size() != numBuckets) {
      throw std::runtime_error(
          "Corrupt estimates file. Mismatch in feerate conf average bucket "
          "count");
    }
  }
  ...
  for (unsigned int i = 0; i < maxPeriods; i++) {
    if (failAvg[i].size() != numBuckets) {
      throw std::runtime_error(
          "Corrupt estimates file. Mismatch in one of failure average bucket "
          "counts");
    }
  }
...
void TxConfirmStats::removeTx(unsigned int entryHeight,
  ...
  if (blocksAgo < 0) {
    ...
    return;  // This can't happen because we call this with our best seen
             // height, no entries can have higher
  }
...
void CBlockPolicyEstimator::processTransaction(const CTxMemPoolEntry& entry,
  ...
  if (txHeight != nBestSeenHeight) {
    ...
    // 1-block reorgs. Ignore txs if BlockPolicyEstimator is not in sync with
    // chainActive.Tip(). It will be synced next time a block is processed.
    return;
  }
  ...
  // Feerates are stored and reported as UNT-per-kb:
...
bool CBlockPolicyEstimator::processBlockTx(unsigned int nBlockHeight,
  ...
  if (blocksToConfirm <= 0) {
    ...
    // a height lower than our greatest seen height
    LogPrint(BCLog::ESTIMATEFEE,
...
void CBlockPolicyEstimator::processBlock(
  ...
  for (const auto& entry : entries) {
    if (processBlockTx(nBlockHeight, entry)) countedTxs++;
  }
...
CFeeRate CBlockPolicyEstimator::estimateRawFee(int confTarget,
  ...
  if (confTarget <= 0 || (unsigned int)confTarget > stats->GetMaxConfirms())
    return CFeeRate(0);
  if (successThreshold > 1) return CFeeRate(0);
...
unsigned int CBlockPolicyEstimator::HighestTargetTracked(
    FeeEstimateHorizon horizon) const {
  switch (horizon) {
    case FeeEstimateHorizon::SHORT_HALFLIFE: {
    ...
    default: {
      throw std::out_of_range(
          "CBlockPolicyEstimator::HighestTargetTracked unknown "
          "FeeEstimateHorizon");
    }
  }
}
 ...
 */
double CBlockPolicyEstimator::estimateConservativeFee(
    unsigned int doubleTarget, EstimationResult* result) const {
  double estimate = -1;
  EstimationResult tempResult;
...
bool CBlockPolicyEstimator::Write(CAutoFile& fileout) const {
  try {
    ...
    LogPrintf(
        "CBlockPolicyEstimator::Write(): unable to write policy estimator data "
        "(non-fatal)\n");
    return false;
  }
  return true;
  ...
  try {
    ...
    if (nVersionRequired < 149900) {
      ...
      if (nFileHistoricalFirst > nFileHistoricalBest ||
        ...
        throw std::runtime_error(
            "Corrupt estimates file. Historical block range for estimates is "
            "invalid");
      }
      ...
      if (numBuckets <= 1 || numBuckets > 1000)
        throw std::runtime_error(
            "Corrupt estimates file. Must have between 2 and 1000 feerate "
            "buckets");
    ...
    LogPrintf(
        "CBlockPolicyEstimator::Read(): unable to read policy estimator data "
        "(non-fatal): %s\n",
        e.what());
    return false;
  }
