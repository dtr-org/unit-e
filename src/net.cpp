...
void SetLimited(enum Network net, bool fLimited) {
  if (net == NET_UNROUTABLE || net == NET_INTERNAL) return;
  LOCK(cs_mapLocalHost);
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char* pszDest,
  ...
  if (pszDest) {
    ...
    if (Lookup(pszDest, resolved, default_port, fNameLookup && !HaveNameProxy(),
      ...
      // resolved to. In that case, drop the connection that was just created,
      // and return the existing CNode instead. Also store the name we used to
      // connect in that CNode, so that future FindNode() calls to that name
      // catch this early.
      LOCK(cs_vNodes);
      CNode* pnode = FindNode((CService)addrConnect);
      if (pnode) {
  ...
  if (addrConnect.IsValid()) {
    ...
    if (!proxyConnectionFailed) {
      // If a connection to the node was attempted, and failure (if any) is not
      // caused by a problem connecting to the proxy, mark this as an attempt.
...
void CNode::copyStats(CNodeStats& stats) {
  ...
  // (UnitE users should be well used to small numbers with many decimal places
  // by now :)
...
void CNode::SetSendVersion(int nVersionIn) {
  ...
  if (nSendVersion != 0) {
    error(
        "Send version already set for node: %i. Refusing to change from %i to "
        "%i",
...
bool CConnman::AttemptToEvictConnection() {
  ...
  // longest. This replicates the non-eviction implicit behavior, and precludes
  // attacks that start later.
  ...
  while (!interruptNet) {
...

    {
      ...
      for (CNode* pnode : vNodes) {
        ...
        //   themselves receiving data. This means properly utilizing TCP flow
        //   control signalling.
...
void ThreadMapPort() {
  ...
  if (r == 1) {
    ...
    std::string strDesc = "UnitE " + FormatFullVersion();
...
void CConnman::ThreadDNSAddressSeed() {
  ...
  for (const std::string& seed : vSeeds) {
    ...
    if (HaveNameProxy()) {
      ...
      unsigned int nMaxIPs =
          256;  // Limits number of IPs learned from a DNS seed
      if (LookupHost(host.c_str(), vIPs, nMaxIPs, true)) {
        ...
        // We now avoid directly using results from DNS Seeds which do not
        // support service bit filtering, instead using them as a oneshot to get
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect) {
  ...
  while (!interruptNet) {
    ...
    std::set<std::vector<unsigned char>> setConnected;
    {
      ...
      for (CNode* pnode : vNodes) {
        if (!pnode->fInbound && !pnode->m_manual_connection) {
          ...
          // our goal here is to not use multiple of our limited outbound slots
          // on a single netgroup but inbound and addnode peers do not use our
          // outbound slots.  Inbound peers also have the added issue that
          // they're attacker controlled and could be used to prevent us from
    ...
    while (!interruptNet) {
      ...
      // fetched from addrman, stop this loop, and let the outer loop run again
      // (which sleeps, adds seed nodes, recalculates already-connected network
      // ranges, ...) before trying new addrman addresses.
      nTries++;
      if (nTries > 100) break;
...
void CConnman::ThreadOpenAddedConnections() {
  while (true) {
    ...
    for (const AddedNodeInfo& info : vInfo) {
      if (!info.fConnected) {
        if (!grant.TryAcquire()) {
          ...
          // here since while we are waiting the addednodeinfo state might
          // change.
          break;
        }
        tried = true;
    ...
    // seconds
    if (!interruptNet.sleep_for(std::chrono::seconds(tried ? 60 : 2))) return;
  }
}
...
void CConnman::OpenNetworkConnection(const CAddress& addrConnect,
  ...
  if (manual_connection) pnode->m_manual_connection = true;
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError,
  ...
  if (hListenSocket == INVALID_SOCKET) {
    strError = strprintf(
        "Error: Couldn't open socket for incoming connections (socket returned "
        "error %s)",
  ...
  // the option and enable it by default or not. Try to enable it, if possible.
  if (addrBind.IsIPv6()) {
#ifdef IPV6_V6ONLY
#ifdef WIN32
...
void CConnman::Stop() {
  ...
  // Close sockets
  for (CNode* pnode : vNodes) pnode->CloseSocketDisconnect();
