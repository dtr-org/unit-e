...
StakeThread::StakeThread(const std::string &&name, std::thread &thread)
...
bool StakeThread::IsStopped() { return m_stopMinerProcess; }

void StakeThread::Start(size_t nThreadID, std::vector<CWallet *> &vpwallets,
...
void StakeThread::StartStaking(const esperanza::Config &config,
  ...
  for (size_t i = 0; i < wallets.size(); i++) {
    ...
    StakeThread *stakeThread = new StakeThread(std::move(threadName), thread);
