    ...
    if (0 <= opcode && opcode <= OP_PUSHDATA4) {
      if (vch.size() <= static_cast<std::vector<unsigned char>::size_type>(4)) {
        ...
        if (fAttemptSighashDecode && !script.IsUnspendable()) {
          ...
          // looks like a signature within a scriptSig. this won't decode
          // correctly formatted public keys in Pubkey or Multisig scripts due
          // to the restrictions on the pubkey formats (see
          // IsCompressedOrUncompressedPubKey) being incongruous with the checks
          // in CheckSignatureEncoding.
...
void TxToUniv(const CTransaction& tx, const uint256& hashBlock, UniValue& entry,
  ...
  if (include_hex) {
    entry.pushKV(
        "hex",
        EncodeHexTx(
            tx, serialize_flags));  // the hex-encoded transaction. used the
                                    // name "hex" to be consistent with the
