...
class CTxMemPool {
 private:
  ...
  uint64_t totalTxSize;  //!< sum of all mempool tx's virtual sizes. Differs
                         //!< from serialized tx size since witness data is
                         //!< discounted. Defined in BIP 141.
 ...
 public:
   ...
   */
  void RemoveStaged(
      setEntries& stage, bool updateDescendants,
      MemPoolRemovalReason reason = MemPoolRemovalReason::UNKNOWN);
   ...
   * sizelimit. pvNoSpendsRemaining, if set, will be populated with the list of
   * outpoints which are not in mempool which no longer have any spends in this
   * mempool.
   */
