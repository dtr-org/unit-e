...
namespace {
    struct CBlockIndexWorkComparator
    {
        bool operator()(const CBlockIndex *pa, const CBlockIndex *pb) const {
            ...
            if (pa->chain_stake > pb->chain_stake) return false;
            if (pa->chain_stake < pb->chain_stake) return true;
...
private:
    ...
    /** chainstake for the last block that preciousblock has been applied to. */
    arith_uint256 nLastPreciousChainstake = 0;
...
SyncStatus GetInitialBlockDownloadStatus()
{
    ...
    if (chainActive.Tip()->chain_stake < nMinimumChainWork) {
...
static void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->chain_stake > chainActive.Tip()->chain_stake + (GetBlockProof(*chainActive.Tip()) * 6)))
    {
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    if (pfork && (!pindexBestForkTip || pindexNewForkTip->nHeight > pindexBestForkTip->nHeight) &&
            pindexNewForkTip->chain_stake - pfork->chain_stake > (GetBlockProof(*pfork) * 7) &&
...
{
    if (!pindexBestInvalid || pindexNew->chain_stake > pindexBestInvalid->chain_stake)
        pindexBestInvalid = pindexNew;

    LogPrintf("%s: invalid block=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
      log(pindexNew->chain_stake.getdouble())/log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S",
      pindexNew->GetBlockTime()));
    CBlockIndex *tip = chainActive.Tip();
    assert (tip);
    LogPrintf("%s:  current best=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
      tip->GetBlockHash().ToString(), chainActive.Height(), log(tip->chain_stake.getdouble())/log(2.0),
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != mapBlockIndex.end()) {
            if (it->second->GetAncestor(pindex->nHeight) == pindex &&
                ...
                pindexBestHeader->chain_stake >= nMinimumChainWork) {
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
      ...
      log(pindexNew->chain_stake.getdouble())/log(2.0), (unsigned long)pindexNew->nChainTx,
    ...
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            if (fFailedChain || fMissingData) {
                ...
                if (fFailedChain && (pindexBestInvalid == nullptr || pindexNew->chain_stake > pindexBestInvalid->chain_stake))
                    pindexBestInvalid = pindexNew;
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        for (CBlockIndex *pindexConnect : reverse_iterate(vpindexToConnect)) {
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                ...
                if (!pindexOldTip || chainActive.Tip()->chain_stake > pindexOldTip->chain_stake) {
...
bool CChainState::PreciousBlock(CValidationState& state, const CChainParams& params, CBlockIndex *pindex)
{
    {
        LOCK(cs_main);
        if (pindex->chain_stake < chainActive.Tip()->chain_stake) {
            // Nothing to do, this block is not at the tip.
            return true;
        }
        if (chainActive.Tip()->chain_stake > nLastPreciousChainstake) {
            // The chain has been extended since the last call, reset the counter.
            nBlockReverseSequenceId = -1;
        }
        nLastPreciousChainstake = chainActive.Tip()->chain_stake;
        setBlockIndexCandidates.erase(pindex);
...
CBlockIndex* CChainState::AddToBlockIndex(const CBlockHeader& block)
{
    ...
    pindexNew->chain_stake = (pindexNew->pprev ? pindexNew->pprev->chain_stake : 0) + GetBlockProof(*pindexNew);
    pindexNew->RaiseValidity(BLOCK_VALID_TREE);
    if (pindexBestHeader == nullptr || pindexBestHeader->chain_stake < pindexNew->chain_stake)
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    bool fHasMoreOrSameWork = (chainActive.Tip() ? pindex->chain_stake >= chainActive.Tip()->chain_stake : true);
    ...
    if (!fRequested) {  // If we didn't ask for it:
        ...
        if (pindex->chain_stake < nMinimumChainWork) return true;
    }
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    {
        CBlockIndex* pindex = item.second;
        pindex->chain_stake = (pindex->pprev ? pindex->pprev->chain_stake : 0) + GetBlockProof(*pindex);
        ...
        if (pindex->IsValid(BLOCK_VALID_TRANSACTIONS) && (pindex->nChainTx || pindex->pprev == nullptr))
            setBlockIndexCandidates.insert(pindex);
        if (pindex->nStatus & BLOCK_FAILED_MASK && (!pindexBestInvalid || pindex->chain_stake > pindexBestInvalid->chain_stake))
...
void CChainState::CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != nullptr) {
        ...
        assert(pindex->pprev == nullptr || pindex->chain_stake >= pindex->pprev->chain_stake); // For every block except the genesis block, the chainwork must be larger than the parent's.
