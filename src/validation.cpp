...
class CChainState {
 private:
   ...
   * itself and all ancestors) and as good as our current tip or better. Entries
   * may be failed, though, and pruning nodes may be missing the data for the
   * block.
   */
...
bool TestLockPointValidity(const LockPoints* lp) {
  ...
  if (lp->maxInputBlock) {
    ...
    // LockPoints calculation was valid.  If not LockPoints are no longer valid
...
static bool AcceptToMemoryPoolWorker(
  ...
  AssertLockHeld(cs_main);
  LOCK(pool.cs);  // mempool "read lock" (held through
  ...
  if (tx.IsCoinBase()) return state.DoS(100, false, REJECT_INVALID, "coinbase");
  ...
  // A transaction with 1 segwit input and 1 P2WPHK output has non-witness size
  // of 82 bytes. Transactions smaller than this are not relayed to reduce
...

  {
    ...
    // we have all inputs cached now, so switch back to dummy, so we don't need
    // to keep lock on mempool
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, view, pool,
      ...
      if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
        ...
        if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS,
                         true, false, txdata)) {
          return error(
              "%s: ConnectInputs failed against MANDATORY but not STANDARD "
              "flags due to promiscuous mempool %s, %s",
              __func__, hash.ToString(), FormatStateMessage(state));
        } else {
          LogPrintf(
              "Warning: -promiscuousmempool flags set to not include currently "
              "enforced soft forks, this may break mining or otherwise cause "
              "instability!\n");
        }
      }
    }
    ...
    for (const CTxMemPool::txiter it : allConflicting) {
      LogPrint(BCLog::MEMPOOL,
               "replacing tx %s with %s for %s UNT additional fees, %d delta "
               "bytes\n",
               it->GetTx().GetHash().ToString(), hash.ToString(),
               FormatMoney(nModifiedFees - nConflictingFees),
               (int)nSize - (int)nConflictingSize);
      if (plTxnReplaced) plTxnReplaced->push_back(it->GetSharedTx());
    }
    pool.RemoveStaged(allConflicting, false, MemPoolRemovalReason::REPLACED);

    // This transaction should only count for fee estimation if:
    // - it isn't a BIP 125 replacement transaction (may not be widely
    // supported)
    // - it's not being readded during a reorg which bypasses typical mempool
    // fee limits
    // - the node is not behind
    // - the transaction is not dependent on any other transactions in the
    // mempool
...
static bool AcceptToMemoryPoolWithTime(
  ...
  if (!res) {
    for (const COutPoint& hashTx : coins_to_uncache) pcoinsTip->Uncache(hashTx);
  }
...
static bool WriteBlockToDisk(
    const CBlock& block, CDiskBlockPos& pos,
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex,
  ...
  if (!ReadBlockFromDisk(block, blockPos, consensusParams)) return false;
  if (block.GetHash() != pindex->GetBlockHash())
    return error(
        "ReadBlockFromDisk(CBlock&, CBlockIndex*): GetHash() doesn't match "
        "index for %s at %s",
...
bool IsInitialBlockDownload() {
  ...
  if (chainActive.Tip()->GetBlockTime() < (GetTime() - nMaxTipAge)) return true;
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip) {
  ...
  // it) of ours We use 7 blocks rather arbitrarily as it represents just under
  // 10% of sustained network hash rate operating on the fork. or a chain that
  // is entirely longer than ours and invalid (note that this should be detected
  // by both) We define it this way because it allows us to only store the
  // highest fork tip (+ base) which meets the 7-block condition and from this
  // always have the most-likely-to-cause-warning fork
  if (pfork &&
      (!pindexBestForkTip ||
...
void InitScriptExecutionCache() {
  ...
  LogPrintf(
      "Using %zu MiB out of %zu/2 requested for script execution cache, able "
      "to store %zu elements\n",
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out) {
  ...
  // we know for sure that the coin did not already exist in the cache. As we
  // have queried for that above using HaveCoin, we don't need to guess. When
  // fClean is false, a coin already existed and it is an overwrite.
...
static unsigned int GetBlockScriptFlags(
...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state,
  ...
  if (!hashAssumeValid.IsNull()) {
    ...
    if (it != mapBlockIndex.end()) {
      if (it->second->GetAncestor(pindex->nHeight) == pindex &&
        ...
        // of the best header. The equivalent time check discourages hash power
  ...
  if (!WriteTxIndexDataForBlock(block, state, pindex)) return false;
...
bool CChainState::ActivateBestChain(CValidationState& state,
  ...
  // e.g. the callback queue to drain we use m_cs_chainstate to enforce mutual
  // exclusion so that only one caller may execute this function at a time
  ...
  do {
...

    {
      ...
      do {
        ...
        // progress (with the exception of shutdown due to hardware issues, low
        // disk space, etc).
        ConnectTrace connectTrace(
        ...
        for (const PerBlockConnectTrace& trace :
          ...
          GetMainSignals().BlockConnected(trace.pblock, trace.pindex,
               ...
               (starting_tip &&
                CBlockIndexWorkComparator()(chainActive.Tip(), starting_tip)));
      ...
      // Enqueue while holding cs_main to ensure that UpdatedBlockTip is called
      // in the order in which blocks are connected
    ...
    // We check shutdown only after giving ActivateBestChainStep a chance to run
    // once so that we never shutdown before connecting the genesis block during
    // LoadChainTip(). Previously this caused an assert() failure during
    // shutdown in such cases as the UTXO DB flushing checks that the best block
    // hash is non-null.
...
bool CheckBlock(const CBlock& block, CValidationState& state,
  ...
  if (!CheckBlockHeader(block, state, consensusParams, fCheckPOW)) return false;
...
std::vector<unsigned char> GenerateCoinbaseCommitment(
    CBlock& block, const CBlockIndex* pindexPrev,
    const Consensus::Params& consensusParams) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block,
  ...
  // Reject outdated version blocks when 95% (75% on testnet) of the network has
  // upgraded: check for version 2, 3 and 4 upgrades
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state,
  ...
  //   root, witness nonce). In case there are multiple, the last one is used.
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers,
  ...
  if (first_invalid != nullptr) first_invalid->SetNull();
  {
    ...
    for (const CBlockHeader& header : headers) {
      CBlockIndex* pindex = nullptr;  // Use a temp pindex instead of ppindex to
                                      // avoid a const_cast
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock,
  ...
  // not being such a candidate itself.
...
static FILE* OpenDiskFile(const CDiskBlockPos& pos, const char* prefix,
  ...
  if (!file && !fReadOnly) file = fsbridge::fopen(path, "wb+");
  if (!file) {
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params,
  ...
  for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight) {
    ...
    // some point. Pruned nodes may have deleted the block.
    if (pindex->nTx > 0) {
      if (pindex->pprev) {
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView* coinsview,
  ...
  for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev;
    ...
    if (fPruneMode && !(pindex->nStatus & BLOCK_HAVE_DATA)) {
      ...
      LogPrintf(
          "VerifyDB(): block verification stopping at height %d (pruning, no "
          "data)\n",
          pindex->nHeight);
      break;
    }
    ...
    if (nCheckLevel >= 3 && pindex == pindexState &&
      ...
      if (res == DISCONNECT_FAILED) {
        return error(
            "VerifyDB(): *** irrecoverable inconsistency in block data at %d, "
            "hash=%s",
            pindex->nHeight, pindex->GetBlockHash().ToString());
      }
  ...
  if (pindexFailure)
    return error(
        "VerifyDB(): *** coin database inconsistencies found (last %i blocks, "
        "%i good transactions before that)\n",
...
bool CChainState::RewindBlockIndex(const CChainParams& params) {
  ...
  while (chainActive.Height() >= nHeight) {
    ...
    if (!FlushStateToDisk(params, state, FLUSH_STATE_PERIODIC)) return false;
  }

  // Reduce validity flag and have-data flags.
  // We do this after actual disconnecting, otherwise we'll end up writing the
  // lack of data to disk before writing the chainstate, resulting in a failure
  // to continue if interrupted.
  for (const auto& entry : mapBlockIndex) {
...
bool CChainState::LoadGenesisBlock(const CChainParams& chainparams) {
  ...
  if (mapBlockIndex.count(chainparams.GenesisBlock().GetHash())) return true;
  ...
  try {
    ...
    while (!blkdat.eof()) {
      ...
      try {
        ...
        // read size
        blkdat >> nSize;
        if (nSize < 80 || nSize > MAX_BLOCK_SERIALIZED_SIZE) continue;
...
void CChainState::CheckBlockIndex(const Consensus::Params& consensusParams) {
  ...
  // ActivateBestChain, so we have the genesis block in mapBlockIndex but no
  // active chain.  (A few of the tests when iterating the block tree require
  // that chainActive has been initialized.)
  ...
  CBlockIndex* pindexFirstMissing = nullptr;  // Oldest ancestor of pindex which
                                              // does not have BLOCK_HAVE_DATA.
  ...
  CBlockIndex* pindexFirstNotTreeValid =
      nullptr;  // Oldest ancestor of pindex which does not have
                // BLOCK_VALID_TREE (regardless of being valid or not).
  ...
  while (pindex != nullptr) {
    ...
    if (pindex->pprev == nullptr) {
      ...
      assert(pindex ==
             chainActive.Genesis());  // The current active chain's genesis
                                      // block must be this block.
    }
    if (pindex->nChainTx == 0)
      assert(pindex->nSequenceId <=
             0);  // nSequenceId can't be set positive for blocks that aren't
                  // linked (negative is used for preciousblock)
    ...
    assert((pindexFirstNeverProcessed != nullptr) ==
           (pindex->nChainTx ==
            0));  // nChainTx != 0 is used to signal that all parent blocks have
                  // been processed (but may have been pruned).
    ...
    if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) &&
      ...
      if (pindexFirstInvalid == nullptr) {
        ...
        // data. In this case it must be in mapBlocksUnlinked -- see test below.
      }
    ...
    if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) &&
      ...
      // in setBlockIndexCandidates, then it must be in mapBlocksUnlinked.
      if (!CBlockIndexWorkComparator()(pindex, chainActive.Tip()) &&
    ...
    while (pindex) {
      ...
      while (rangePar.first->second != pindex) {
        assert(rangePar.first !=
               rangePar.second);  // Our parent must have at least the node
                                  // we're coming from as child.
        rangePar.first++;
      }
...
bool LoadMempool(void) {
  ...
  LogPrintf(
      "Imported mempool transactions from disk: %i succeeded, %i failed, %i "
      "expired, %i already there\n",
