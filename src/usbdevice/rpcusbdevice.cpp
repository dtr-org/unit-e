...
static UniValue getdevicepubkey(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"account_path\"      (string, optional) Account path, set to empty string to ignore (default=\""+ DEFAULT_ACCOUNT_PATH +"\").\n"
            "\nResult\n"
            "{\n"
            ...
            + HelpExampleCli("getdevicepubkey", "\"0/0\"")
...
static UniValue initaccountfromdevice(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "derive the key from (default=\""+ DEFAULT_ACCOUNT_PATH +"\").\n"
            "\nResult\n"
            "{\n"
