...
class TorControlConnection {
 public:
   ...
   * established. disconnected is a handler that is called when the connection
   * is broken. Return true on success.
   */
...
void TorControlConnection::readcb(struct bufferevent *bev, void *ctx) {
  ...
  //  long lines Do this after evbuffer_readln to make sure all full lines have
  //  been removed from the buffer. Everything left is an incomplete line.
 ...
 */
static std::pair<std::string, std::string> SplitTorReplyLine(
    const std::string &s) {
  size_t ptr = 0;
  std::string type;
 ...
 * COOKIEFILE=".../control_auth_cookie"'. Returns a map of keys to values, or an
 * empty map if there was an error. Grammar is implicitly defined in
 * https://spec.torproject.org/control-spec by the server reply formats for
 * PROTOCOLINFO (S3.21), AUTHCHALLENGE (S3.24), and ADD_ONION (S3.27). See also
 * sections 2.1 and 2.3.
 */
static std::map<std::string, std::string> ParseTorReplyMapping(
    const std::string &s) {
 ...
 * file is larger than this, truncated data (with len > maxsize) will be
 * returned.
 */
static std::pair<bool, std::string> ReadBinaryFile(
...
/****** UnitE specific TorController implementation ********/
...
void TorController::add_onion_cb(TorControlConnection &_conn,
  ...
  if (reply.code == 250) {
    ...
    LogPrintf(
        "tor: Add onion failed with unrecognized command (You probably need to "
        "upgrade Tor)\n");
  } else {
...
void TorController::auth_cb(TorControlConnection &_conn,
  ...
  if (reply.code == 250) {
    ...
    // port, but this is just a convenient choice.  TODO; refactor the shutdown
    // sequence some day.
...
static std::vector<uint8_t> ComputeResponse(
    const std::string &key, const std::vector<uint8_t> &cookie,
...
void TorController::protocolinfo_cb(TorControlConnection &_conn,
  ...
  if (reply.code == 250) {
     ...
     * COOKIEFILE="/home/x/.tor/control_auth_cookie" 250-AUTH METHODS=NULL
    ...
    if (!torpassword.empty()) {
      if (methods.count("HASHEDPASSWORD")) {
        ...
        LogPrintf(
            "tor: Password provided with -torpassword, but HASHEDPASSWORD "
      ...
      if (status_cookie.first &&
        ...
        if (status_cookie.first) {
          LogPrintf(
              "tor: Authentication cookie %s is not exactly %i bytes, as is "
              "required by the spec\n",
              cookiefile, TOR_COOKIE_SIZE);
        } else {
          LogPrintf(
              "tor: Authentication cookie %s could not be opened (check "
              "permissions)\n",
              cookiefile);
        }
      }
      ...
      LogPrintf(
          "tor: The only supported authentication mechanism left is password, "
          "but no password provided with -torpassword\n");
    } else {
