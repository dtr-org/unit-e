...
class CValidationInterface {
 protected:
   ...
   *
   * When multiple blocks are connected at once, UpdatedBlockTip will be called
   * on the final tip but may not be called on every intermediate tip. If the
   * latter behavior is desired, subscribe to BlockConnected() instead.
   *
...
class CMainSignals {
 ...
 public:
  ...
  void BlockConnected(
      const std::shared_ptr<const CBlock> &, const CBlockIndex *pindex,
