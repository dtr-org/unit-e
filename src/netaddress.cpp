...
int CNetAddr::GetReachabilityFrom(const CNetAddr* paddrPartner) const {
  ...
  if (!IsRoutable() || IsInternal()) return REACH_UNREACHABLE;
      ...
      switch (ourNet) {
        ...
        case NET_IPV6:
          return fTunnel ? REACH_IPV6_WEAK
                         : REACH_IPV6_STRONG;  // only prefer giving our IPv6
                                               // address if it's not tunnelled
      }
    case NET_TOR:
      switch (ourNet) {
        default:
    ...
    default:
      switch (ourNet) {
        ...
        case NET_IPV4:
          return REACH_IPV4;
        case NET_TOR:
          return REACH_PRIVATE;  // either from Tor, or don't care about our
                                 // address
      }
  }
}
...
CSubNet::CSubNet(const CNetAddr& addr, const CNetAddr& mask) {
  ...
  for (int x = astartofs; x < 16; ++x) netmask[x] = mask.ip[x];
