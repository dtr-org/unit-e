 ...
 * is shared between united, and unite-qt and non-server tools.
 */
 ...
 public:
  ...
  bool GetCoin(const COutPoint& outpoint, Coin& coin) const override {
    try {
      ...
      // be interpreted as 'entry not found' (as opposed to unable to read
      // data), and could lead to invalid interpretation. Just exit immediately,
      // as we can't continue anyway, and all writes should be atomic.
      abort();
    }
  }
...
std::string HelpMessage(HelpMessageMode mode) {
  ...
  strUsage += HelpMessageOpt(
      "-pid=<file>",
      strprintf(_("Specify pid file (default: %s)"), UNITE_PID_FILENAME));
#endif
  strUsage += HelpMessageOpt(
      "-prune=<n>",
...
void CleanupBlockRevFiles() {
  ...
  LogPrintf(
      "Removing unusable blk?????.dat and rev?????.dat files for -reindex with "
      "-prune\n");
...
void InitParameterInteraction() {
  ...
  if (gArgs.GetBoolArg("-whitelistforcerelay", DEFAULT_WHITELISTFORCERELAY)) {
    if (gArgs.SoftSetBoolArg("-whitelistrelay", true)) {
      LogPrintf(
          "%s: parameter interaction: -whitelistforcerelay=1 -> setting "
          "-whitelistrelay=1\n",
          __func__);
    }
  }
...
bool AppInitBasicSetup() {
  ...
  // 0x0601 (Windows 7), which is not correct. Can be removed, when GCCs
  // winbase.h is fixed!
...
bool AppInitParameterInteraction() {
  ...
  if (gArgs.GetBoolArg("-whitelistalwaysrelay", false)) {
    InitWarning(
        _("Unsupported argument -whitelistalwaysrelay ignored, use "
  ...
  // 125 replacement in the mempool and the amount the mempool min fee increases
  // above the feerate of txs evicted due to mempool limiting.
    ...
    for (const std::string& strDeployment : gArgs.GetArgs("-vbparams")) {
      ...
      for (int j = 0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
        if (vDeploymentParams[0].compare(VersionBitsDeploymentInfo[j].name) ==
          ...
          found = true;
          LogPrintf(
              "Setting version bits activation parameters for %s to start=%ld, "
              "timeout=%ld\n",
...
bool AppInitMain() {
  ...
  if (gArgs.IsArgSet("-datadir") &&
    ...
    LogPrintf(
        "Warning: relative datadir option '%s' specified, which will be "
        "interpreted relative to the "
        "current working directory '%s'. This is fragile, because if unite is "
        "started in the future "
        "from a different location, it will be unable to locate the current "
        "data files. There could "
        "also be data loss if unite is started while in a temporary "
        "directory.\n",
  ...
  LogPrintf(
      "* Using %.1fMiB for in-memory UTXO set (plus up to %.1fMiB of unused "
      "mempool space)\n",
    ...
    do {
      try {
        ...
        if (fTxIndex != gArgs.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
          strLoadError =
              _("You need to rebuild the database using -reindex to change "
                "-txindex");
          break;
        }
        ...
        // on disk (otherwise we use the one already on disk). This is called
        // again in ThreadImport after the reindex completes.
        ...
        if (!fReset) {
          ...
          // -reindex-chainstate. It both disconnects blocks based on
          // chainActive, and drops block data in mapBlockIndex based on lack of
          // available witness data.
        ...
        if (!is_coinsview_empty) {
          ...
          if (fHavePruned && gArgs.GetArg("-checkblocks", DEFAULT_CHECKBLOCKS) >
                                 MIN_BLOCKS_TO_KEEP) {
            LogPrintf(
                "Prune: pruned datadir may not have more than %d blocks; only "
  ...
  if (chainparams.GetConsensus()
    ...
    // setting its end time to 0. Note that setting NODE_WITNESS is never
    // required: the only downside from not doing so is that after activation,
    // no upgraded nodes will fetch from you.
  ...
  // fHaveGenesis directly. No locking, as this happens before any background
  // thread is started.
