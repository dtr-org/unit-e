 ...
 */
class CUnitESecret : public CBase58Data {
 public:
  ...
  CUnitESecret(const CKey& vchSecret) { SetKey(vchSecret); }
  CUnitESecret() {}
};

template <typename K, int Size, CChainParams::Base58Type Type>
class CUnitEExtKeyBase : public CBase58Data {
 public:
  ...
  CUnitEExtKeyBase(const K& key) { SetKey(key); }
...
typedef CUnitEExtKeyBase<CExtKey, BIP32_EXTKEY_SIZE,
                         CChainParams::EXT_SECRET_KEY>
    CUnitEExtKey;
typedef CUnitEExtKeyBase<CExtPubKey, BIP32_EXTKEY_SIZE,
