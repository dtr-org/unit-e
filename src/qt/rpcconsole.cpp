 ...
 * command(s). Aims to emulate \c bash and friends.
 *
        ...
        switch (ch) {
          ...
          default:
            if (state == STATE_COMMAND_EXECUTED_INNER) {
              if (ch != ']') {
                // append char to the current argument (which is also used for
                // the query command)
                curarg += ch;
                break;
              }
            ...
            // don't break parsing when the char is required for the next
            // argument
            breakParsing = false;

            // pop the stack and return the result to the current command
            // arguments
            close_out_params();
            ...
            // if we have a non empty result, use it as stack argument otherwise
            // as general result
            if (curarg.size()) {
              if (stack.size())
      ...
      case STATE_EATING_SPACES:  // Handle runs of whitespace
        switch (ch) {
          ...
          case '\n':
            ...
            if ((ch == ')' || ch == '\n') && stack.size() > 0) {
              if (fExecute) {
                // Convert argument list to JSON objects in method-dependent
                // way, and pass it along with the method name to the
                // dispatcher.
                JSONRPCRequest req;
          ...
          case ',':
          case '\t':
            if (state == STATE_EATING_SPACES_IN_ARG && curarg.empty() &&
                ch == ',')
              throw std::runtime_error("Invalid Syntax");
...
void RPCExecutor::request(const QString &command) {
  try {
    ...
    // Catch the console-only-help command before RPC call is executed and reply
    // with help text as-if a RPC reply.
  ...
  if (event->type() == QEvent::KeyPress)  // Special key handling
  {
    ...
    switch (key) {
      ...
      case Qt::Key_Enter:
        ...
        break;
      default:
        // Typing in messages widget brings focus to line edit, and redirects
        // key there Exclude most combinations and keys that emit no text,
        // except paste shortcuts
...
void RPCConsole::setClientModel(ClientModel *model) {
  ...
  if (model && clientModel->getPeerTableModel() &&
    ...
    // supports int or objects, which is okay because max bantime (1 year) is <
    // int_max.
