...
#include <config/unite-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update() {
  ...
  QString uri = GUIUtil::formatUnitEURI(info);
  ...
  if (info.amount)
    html +=
        "<b>" + tr("Amount") + "</b>: " +
        UnitEUnits::formatHtmlWithUnit(model->getDisplayUnit(), info.amount) +
        "<br>";
  ...
  if (!uri.isEmpty()) {
    ...
    if (uri.length() > MAX_URI_LENGTH) {
      ui->lblQRCode->setText(
          tr("Resulting URI too long, try to reduce the text for label / "
             "message."));
    } else {
...
void ReceiveRequestDialog::on_btnCopyURI_clicked() {
  GUIUtil::setClipboard(GUIUtil::formatUnitEURI(info));
}
