...
void SendCoinsDialog::setModel(WalletModel *_model) {
  ...
  if (_model && _model->getOptionsModel()) {
    ...
    if (settings.value("nSmartFeeSliderPosition").toInt() != 0) {
      ...
      int nConfirmTarget = 25 - settings.value("nSmartFeeSliderPosition")
...
void SendCoinsDialog::on_sendButton_clicked() {
  ...
  processSendCoinsReturn(
      prepareStatus,
      UnitEUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
  ...
  for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients()) {
    ...
    QString amount =
        "<b>" + UnitEUnits::formatHtmlWithUnit(
  ...
  if (txFee > 0) {
    ...
    questionString.append(UnitEUnits::formatHtmlWithUnit(
  ...
  questionString.append(
      tr("Total Amount %1")
          .arg(UnitEUnits::formatHtmlWithUnit(
...
void SendCoinsDialog::processSendCoinsReturn(
  ...
  // WalletModel::SendCoinsReturn. WalletModel::TransactionCommitFailed is used
  // only in WalletModel::sendCoins() all others are used only in
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text) {
  if (model && model->getAddressTableModel()) {
    ...
    {
      ui->labelCoinControlChangeLabel->setText(
          tr("Warning: Invalid UnitE address"));
    } else  // Valid address
    {
