...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(
  ...
  for (const SendCoinsRecipient &rcp : recipients) {
    ...
    if (rcp.paymentRequest.IsInitialized()) {  // PaymentRequest...
      ...
      total += subtotal;
    } else {  // User-entered unite address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(
...

  {
    ...
    for (const SendCoinsRecipient &rcp : transaction.getRecipients()) {
      ...
      } else if (!rcp.message.isEmpty())  // Message from normal unite:URI
                                          // (unite:123...?message=example)
  ...
  checkBalanceChanged();  // update balance immediately, otherwise there could
                          // be a short noticeable delay until
...
bool WalletModel::bumpFee(uint256 hash) {
  ...
  if (feebumper::CreateTransaction(wallet, hash, coin_control, 0 /* totalFee */,
                                   errors, old_fee, new_fee,
