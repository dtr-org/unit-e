...
#ifndef UNITE_QT_GUIUTIL_H
#define UNITE_QT_GUIUTIL_H
...
/** Utility functions used by the UnitE Qt UI.
 */
namespace GUIUtil {
...
// Parse "unite:" URI into recipient object, return true on successful parsing
...
/** Get save filename, mimics QFileDialog::getSaveFileName, except that it
  appends a default suffix when no suffix is provided by the user.
  ...
  was selected (or 0). Can be useful when choosing the save file format based on
  suffix.
 */
 ...
 * border. Also makes sure the column widths are never larger than the table's
 * viewport. In Qt, all columns are resizable from the right, but it's not
 * intuitive resizing the last column from the right. Usually our second to last
 * columns behave as if stretched, and when on strech mode, columns aren't
 * resizable interactively or programmatically.
 *
...
class ClickableProgressBar : public QProgressBar {
 ...
 Q_SIGNALS:
   ...
   * the click are passed to the signal.
   */
  void clicked(const QPoint &point);
