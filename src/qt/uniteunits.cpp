...
QString UnitEUnits::longName(int unit) {
  switch (unit) {
    ...
    case mUNT:
      return QString("mUNT");
    case uUNT:
      return QString::fromUtf8("µUNT (bits)");
    default:
      return QString("???");
  }
}
...
QString UnitEUnits::description(int unit) {
  switch (unit) {
    case UNT:
      return QString("UnitEs");
    case mUNT:
      return QString("Milli-UnitEs (1 / 1" THIN_SP_UTF8 "000)");
    case uUNT:
      return QString("Micro-UnitEs (bits) (1 / 1" THIN_SP_UTF8
...
QString UnitEUnits::format(int unit, const CAmount &nIn, bool fPlus,
...
QString UnitEUnits::formatWithUnit(int unit, const CAmount &amount,
                                   bool plussign, SeparatorStyle separators) {
  return format(unit, amount, plussign, separators) + QString(" ") +
         shortName(unit);
}

QString UnitEUnits::formatHtmlWithUnit(int unit, const CAmount &amount,
                                       bool plussign,
                                       SeparatorStyle separators) {
...
CAmount UnitEUnits::maxMoney() { return MAX_MONEY; }
