...
static AddressTableEntry::Type translateTransactionType(
    const QString &strPurpose, bool isMine) {
...
class AddressTablePriv {
 public:
  ...
  void refreshAddressTable() {
    ...
    // sorted in asc order Even though the map is already sorted this re-sorting
    // step is needed because the originating map is sorted by binary address,
    // not by base58() address.
  ...
  void updateEntry(const QString &address, const QString &label, bool isMine,
    ...
    switch (status) {
      case CT_NEW:
        if (inModel) {
          qWarning() << "AddressTablePriv::updateEntry: Warning: Got CT_NEW, "
                        "but entry is already in model";
          break;
        }
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value,
  ...
  if (role == Qt::EditRole) {
    ...
    if (index.column() == Label) {
      ...
      // addresses, if you try to paste an existing address over another address
...
void AddressTableModel::updateEntry(const QString &address,
                                    ...
                                    const QString &purpose, int status) {
  // Update address book model from UnitE core
