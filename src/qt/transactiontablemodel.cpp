  ...
  void updateWallet(const uint256 &hash, int status, bool showTransaction) {
    ...
    switch (status) {
      ...
      case CT_DELETED:
        ...
        break;
      case CT_UPDATED:
        // Miscellaneous updates -- nothing to do, status update will take care
        // of this, and is only computed for visible transactions.
...
TransactionTableModel::TransactionTableModel(
  ...
  columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label")
          << UnitEUnits::getAmountColumnTitle(
...
void TransactionTableModel::updateAmountColumnTitle() {
  columns[Amount] = UnitEUnits::getAmountColumnTitle(
      walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxStatus(
...
QString TransactionTableModel::formatTxDate(
    const TransactionRecord *wtx) const {
  if (wtx->time) {
...
QString TransactionTableModel::formatTxType(
    const TransactionRecord *wtx) const {
  switch (wtx->type) {
...
QVariant TransactionTableModel::addressColor(
    const TransactionRecord *wtx) const {
...
QString TransactionTableModel::formatTxAmount(
    ...
    UnitEUnits::SeparatorStyle separators) const {
  QString str =
      UnitEUnits::format(walletModel->getOptionsModel()->getDisplayUnit(),
...
QString TransactionTableModel::formatTooltip(
    const TransactionRecord *rec) const {
  ...
  switch (role) {
    ...
    case Qt::DisplayRole:
      switch (index.column()) {
        ...
        case Amount:
          return formatTxAmount(rec, true, UnitEUnits::separatorAlways);
      }
      break;
    case Qt::EditRole:
    ...
    case TxPlainTextRole: {
      ...
      details.append(formatTxAmount(rec, false, UnitEUnits::separatorNever));
      return details;
    }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
      ...
      return formatTxAmount(rec, false, UnitEUnits::separatorNever);
    case StatusRole:
  ...
  if (orientation == Qt::Horizontal) {
    if (role == Qt::DisplayRole) {
      ...
      switch (section) {
        ...
        case Watchonly:
          return tr(
              "Whether or not a watch-only address is involved in this "
              "transaction.");
        case ToAddress:
