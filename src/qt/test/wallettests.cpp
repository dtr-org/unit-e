...
void TestGUI() {
  ...
  uint256 txid1 =
      SendCoins(wallet, sendCoinsDialog, CKeyID(), 5 * UNIT, false /* rbf */);
  uint256 txid2 =
      SendCoins(wallet, sendCoinsDialog, CKeyID(), 10 * UNIT, true /* rbf */);
  ...
  QString balanceComparison = UnitEUnits::formatWithUnit(
  ...
  // Amount input
  UnitEAmountField* amountInput =
      receiveCoinsDialog.findChild<UnitEAmountField*>("reqAmount");
