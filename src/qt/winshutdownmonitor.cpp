...
bool WinShutdownMonitor::nativeEventFilter(const QByteArray &eventType,
  ...
  if (RAND_event(pMsg->message, pMsg->wParam, pMsg->lParam) == 0) {
    ...
    if (!warned) {
      LogPrintf(
          "%s: OpenSSL RAND_event() failed to seed OpenSSL PRNG with enough "
          "data.\n",
          __func__);
      warned = true;
    }
  ...
  switch (pMsg->message) {
    case WM_QUERYENDSESSION: {
      // Initiate a client shutdown after receiving a WM_QUERYENDSESSION and
      // block Windows session end until we have finished client shutdown.
      StartShutdown();
      *pnResult = FALSE;
      return true;
    }
