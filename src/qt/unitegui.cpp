...
UnitEGUI::UnitEGUI(const PlatformStyle *_platformStyle,
  ...
  // progress bar, as they make the text unreadable (workaround for issue #1071)
...
UnitEGUI::~UnitEGUI() {
  ...
  if (trayIcon)  // Hide tray icon, as deleting will let it linger until quit
                 // (on Ubuntu)
    trayIcon->hide();
#ifdef Q_OS_MAC
  delete appMenuBar;
...
void UnitEGUI::createActions() {
  ...
  receiveCoinsAction->setStatusTip(
      tr("Request payments (generates QR codes and unite: URIs)"));
  ...
  signMessageAction->setStatusTip(
      tr("Sign messages with your UnitE addresses to prove you own them"));
  verifyMessageAction =
      new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                  tr("&Verify message..."), this);
  verifyMessageAction->setStatusTip(
      tr("Verify messages to ensure they were signed with specified UnitE "
         "addresses"));
  ...
  showHelpMessageAction->setStatusTip(
      tr("Show the %1 help message to get a list with possible UnitE "
...
void UnitEGUI::showHelpMessageClicked() { helpMessageDialog->show(); }
...
void UnitEGUI::updateNetworkState() {
  ...
  if (clientModel->getNetworkActive()) {
    tooltip = tr("%n active connection(s) to UnitE network", "", count) +
...
void UnitEGUI::setNumConnections(int count) { updateNetworkState(); }

void UnitEGUI::setNetworkActive(bool networkActive) { updateNetworkState(); }
  ...
  if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC)
    progressBarLabel->setText(
        tr("Syncing Headers (%1%)...")
            .arg(QString::number(
                100.0 / (headersTipHeight + estHeadersLeft) * headersTipHeight,
                'f', 1)));
}

void UnitEGUI::setNumBlocks(int count, const QDateTime &blockDate,
...
void UnitEGUI::message(const QString &title, const QString &message,
...
#ifdef ENABLE_WALLET
void UnitEGUI::incomingTransaction(const QString &date, int unit,
...
void UnitEGUI::toggleHidden() { showNormalIfMinimized(true); }
...
static bool ThreadSafeMessageBox(UnitEGUI *gui, const std::string &message,
