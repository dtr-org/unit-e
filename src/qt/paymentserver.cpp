...
const int UNITE_IPC_CONNECT_TIMEOUT = 1000;  // milliseconds
const QString UNITE_IPC_PREFIX("unite:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/unite-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/unite-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/unite-paymentrequest";
...
void PaymentServer::LoadRootCAs(X509_STORE* _store) {
  ...
  // -rootcertificates="" and get 'I don't like X.509 certificates, don't trust
  // anybody' behavior:
  QString certFile =
  ...
  if (certFile.isEmpty()) {
    qDebug() << QString(
                    "PaymentServer::%1: Payment request authentication via "
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[]) {
  for (int i = 1; i < argc; i++) {
    ...
    // If the unite: URI contains a payment request, we are not able to detect
    // the network as that would require fetching and parsing the payment
    // request. That means clicking such an URI which contains a testnet payment
    // request will start a mainnet instance and throw a "wrong network" error.
    if (arg.startsWith(UNITE_IPC_PREFIX, Qt::CaseInsensitive))  // unite: URI
    {
...
bool PaymentServer::ipcSendCommandLine() {
  ...
  for (const QString& r : savedPaymentRequests) {
    ...
    if (!socket->waitForConnected(UNITE_IPC_CONNECT_TIMEOUT)) {
      delete socket;
      socket = nullptr;
      return false;
    }
    ...
    socket->write(block);
    socket->flush();
    socket->waitForBytesWritten(UNITE_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QObject* parent, bool startLocalServer)
  ...
  // on Mac: sent when you click unite: links
  ...
  if (startLocalServer) {
    ...
    if (!uriServer->listen(name)) {
      ...
      QMessageBox::critical(0, tr("Payment request error"),
                            tr("Cannot start unite: click-to-pay handler"));
    } else {
...
//
// OSX-specific way of handling unite: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager() {
  ...
  // netManager is used to fetch paymentrequests given in unite: URIs
...
void PaymentServer::handleURIOrFile(const QString& s) {
  ...
  if (s.startsWith(UNITE_IPC_PREFIX, Qt::CaseInsensitive))  // unite: URI
  {
#if QT_VERSION < 0x050000
    QUrl uri(s);
#else
    ...
    } else  // normal URI
    {
      ...
      if (GUIUtil::parseUnitEURI(s, &recipient)) {
        ...
        Q_EMIT message(tr("URI handling"),
                       tr("URI cannot be parsed! This can be caused by an "
                          "invalid UnitE address or malformed URI parameters."),
                       CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request,
  ...
  for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
    ...
    if (ExtractDestination(sendingTo.first, dest)) {
      ...
      // Unauthenticated payment requests to custom unite addresses are not
      // supported (there is no good way to tell the user where they are paying
      // in a way they'd have a chance of understanding).
      Q_EMIT message(tr("Payment request rejected"),
    ...
    // UnitE amounts are stored as (optional) uint64 in the protobuf messages
    // (see paymentrequest.proto), but CAmount is defined as int64_t. Because of
    // that we need to verify that amounts are in a valid range and no overflow
    // has happened.
    ...
    if (IsDust(txOut, ::dustRelayFee)) {
      Q_EMIT message(
          ...
          tr("Requested payment amount of %1 is too small (considered dust).")
              .arg(UnitEUnits::formatWithUnit(optionsModel->getDisplayUnit(),
                                              sendingTo.second)),
          CClientUIInterface::MSG_ERROR);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet,
  ...
  if (wallet->GetKeyFromPool(newKey)) {
    // BIP70 requests encode the scriptPubKey directly, so we are not restricted
    // to address types supported by the receiver. As a result, we choose the
    // address format we also use for change. Despite an actual payment and not
    // change, this is a close match: it's the output type we use subject to
    // privacy issues, but not restricted by what other software supports.
...
void PaymentServer::handlePaymentACK(const QString& paymentACKMsg) {
  ...
  Q_EMIT message(
      tr("Payment acknowledged"), paymentACKMsg,
...
bool PaymentServer::verifyNetwork(
  ...
  if (!fVerified) {
    qWarning() << QString(
                      "PaymentServer::%1: Payment request network \"%2\" "
...
bool PaymentServer::verifySize(qint64 requestSize) {
  ...
  if (!fVerified) {
    qWarning() << QString(
                      "PaymentServer::%1: Payment request too large (%2 bytes, "
                      "allowed %3 bytes).")
                      .arg(__func__)
                      .arg(requestSize)
                      .arg(BIP70_MAX_PAYMENTREQUEST_SIZE);
...
bool PaymentServer::verifyAmount(const CAmount& requestAmount) {
  ...
  if (!fVerified) {
    qWarning() << QString(
                      "PaymentServer::%1: Payment request amount out of "
