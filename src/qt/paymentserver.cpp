...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the unite: URI contains a payment request, we are not able to detect the
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("unite://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'unite://' is not a valid URI. Use 'unite:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(UNITE_IPC_PREFIX, Qt::CaseInsensitive)) // unite: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseUnitEURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid UnitE address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom unite addresses are not supported
        ...
        // UnitE amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(UnitEUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
