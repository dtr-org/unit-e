This directory contains the UnitEQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the UnitE UI.
...
ClientModel is used by the main application `unitegui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/unite/unite/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a unite:... link from a browser).
...
* UI elements like UnitEAmountField, which inherit from QWidget.
* `unitestrings.cpp`: automatically generated
* `uniteunits.(h/cpp)`: UNT / mUNT / etc handling
* `callback.h`
...
* don't change `local/unite_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "unite-qt" under "Run", which is where you can also add command line arguments)
