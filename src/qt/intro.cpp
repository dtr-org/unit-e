...
/* Check free space asynchronously to prevent hanging the UI thread.
   ...
   check() function runs, the current path is requested from the associated
   Intro object. The reply is sent back through a signal.

   This ensures that no queue of checking requests is built up while the user is
   still entering the path, and that always the most recently entered path is
   checked as soon as the thread becomes available.
*/
...
Intro::Intro(QWidget *parent)
  ...
  ui->sizeWarningLabel->setText(
      tr("%1 will download and store a copy of the UnitE block chain.")
          .arg(tr(PACKAGE_NAME)) +
...
bool Intro::pickDataDirectory() {
  ...
  if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) ||
    ...
    intro.setWindowIcon(QIcon(":icons/unite"));
   ...
   * to override -datadir in the unite.conf file in the default data directory
   * (to be consistent with united behavior)
   */
  if (dataDir != getDefaultDataDirectory())
