...
CoinControlDialog::CoinControlDialog(const PlatformStyle *_platformStyle,
  ...
  unlockAction = new QAction(tr("Unlock unspent"),
                             this);  // we need to enable/disable this
  ...
  // see https://github.com/unite/unite/issues/5716
...
void CoinControlDialog::showMenu(const QPoint &point) {
  ...
  if (item) {
    ...
    if (item->text(COLUMN_TXHASH).length() ==
        64)  // transaction hash is 64 characters (this means its a child node,
...
void CoinControlDialog::copyAmount() {
  GUIUtil::setClipboard(
      UnitEUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
    ...
    if (sortColumn == logicalIndex)
      ...
      sortOrder =
          ((sortColumn == COLUMN_LABEL || sortColumn == COLUMN_ADDRESS)
               ? Qt::AscendingOrder
               : Qt::DescendingOrder);  // if label or address then default =>
                                        // asc, else default => desc
    }
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem *item, int column) {
  if (column == COLUMN_CHECKBOX &&
      item->text(COLUMN_TXHASH).length() ==
          64)  // transaction hash is 64 characters (this means its a child
               // node, so its not a parent node in tree mode)
  {
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
  ...
  if (nQuantity > 0) {
    ...
    if (fWitness) {
      ...
      // exact. usually, the result will be an overestimate within a couple of
      // satoshis so that the confirmation dialog ends up displaying a slightly
      // smaller fee. also, the witness stack size value is a variable sized
      // integer. usually, the number of stack items will be well under the
      // single byte var int limit.
      nBytes += 2;          // account for the serialized marker and flag bytes
      nBytes += nQuantity;  // account for the witness byte that holds the
                            // number of stack items for each input.
    }
  ...
  int nDisplayUnit = UnitEUnits::UNT;
  ...
  l2->setText(UnitEUnits::formatWithUnit(nDisplayUnit, nAmount));  // Amount
  l3->setText(UnitEUnits::formatWithUnit(nDisplayUnit, nPayFee));  // Fee
  l4->setText(
      UnitEUnits::formatWithUnit(nDisplayUnit, nAfterFee));  // After Fee
  l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") +
              QString::number(nBytes));                            // Bytes
  l7->setText(fDust ? tr("yes") : tr("no"));                       // Dust
  l8->setText(UnitEUnits::formatWithUnit(nDisplayUnit, nChange));  // Change
  if (nPayFee > 0) {
...
void CoinControlDialog::updateView() {
  ...
  ui->treeWidget->setEnabled(
      false);  // performance, otherwise updateLabels would be called for every
               // checked checkbox
  ...
  for (const std::pair<QString, std::vector<COutput>> &coins : mapCoins) {
    ...
    for (const COutput &out : coins.second) {
      ...
      if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey,
        ...
        // if listMode or change => show unite address. In tree mode, address is
        // not shown again for direct wallet address outputs
      ...
      itemOutput->setText(
          COLUMN_AMOUNT,
          UnitEUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
      itemOutput->setData(
    ...
    if (treeMode) {
      ...
      itemWalletAddress->setText(COLUMN_AMOUNT,
                                 UnitEUnits::format(nDisplayUnit, nSum));
