...
ReadStatus PartiallyDownloadedBlock::InitData(
  ...
  for (size_t i = 0; i < cmpctblock.prefilledtxn.size(); i++) {
    ...
    if ((uint32_t)lastprefilledindex > cmpctblock.shorttxids.size() + i) {
      ...
      // shorttxids plus the number of prefilled txn we've inserted, then we
      // have txn for which we have neither a prefilled txn or a shorttxid!
      return READ_STATUS_INVALID;
    }
  ...
  // (or don't) Because well-formed cmpctblock messages will have a (relatively)
  // uniform distribution of short IDs, any highly-uneven distribution of
  // elements can be safely treated as a READ_STATUS_FAILED.
  ...
  for (size_t i = 0; i < cmpctblock.shorttxids.size(); i++) {
    ...
    // is equal to S (due to std::unordered_map having a default load factor
    // of 1.0), and that the chance for any bucket to exceed N elements is at
    // most buckets * (the chance that any given bucket is above N elements).
    // Thus: P(max_elements_per_bucket > N) <= S * (1 - cdf(binomial(n=S,p=1/S),
    // N)). If we assume blocks of up to 16000, allowing 12 elements per bucket
    ...
    for (size_t i = 0; i < vTxHashes.size(); i++) {
      ...
      if (idit != shorttxids.end()) {
        if (!have_txn[idit->second]) {
          ...
          // it. This should be rare enough that the extra bandwidth doesn't
      ...
      // case, the performance win of an early exit here is too good to pass up
      // and worth the extra risk.
