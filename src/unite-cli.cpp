...
std::string HelpMessageCli() {
  ...
  strUsage +=
      HelpMessageOpt("-rpcwallet=<walletname>",
                     _("Send RPC for non-default wallet on RPC server "
                       "(argument is wallet filename in united directory, "
                       "required if united/-Qt runs with multiple wallets)"));
...
static int AppInitRPC(int argc, char* argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    if (!gArgs.IsArgSet("-version")) {
      strUsage += "\n" + _("Usage:") + "\n" +
                  "  unite-cli [options] <command> [params]  " +
                  strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                  "  unite-cli [options] -named <command> [name=value] ... " +
                  strprintf(_("Send command to %s (with named arguments)"),
                            _(PACKAGE_NAME)) +
                  "\n" + "  unite-cli [options] help                " +
                  _("List commands") + "\n" +
                  "  unite-cli [options] help <command>      " +
...
class GetinfoRequestHandler : public BaseRequestHandler {
 public:
  ...
  UniValue ProcessReply(const UniValue& batch_in) override {
    ...
    // Errors in getnetworkinfo() and getblockchaininfo() are fatal, pass them
    // on getwalletinfo() is allowed to fail in case there is no wallet.
  ...
  if (gArgs.GetArg("-rpcpassword", "") == "") {
    ...
    if (!GetAuthCookie(&strRPCUserColonPass)) {
      throw std::runtime_error(strprintf(
          ...
          GetConfigFile(gArgs.GetArg("-conf", UNITE_CONF_FILENAME))
              .string()
              .c_str()));
    }
  } else {
        ...
        if (!error.isNull()) {
          ...
          if (error.isObject()) {
            ...
            if (errCode.isNum() &&
              ...
              strPrint +=
                  "\nTry adding \"-rpcwallet=<filename>\" option to unite-cli "
                  "command line.";
            }
          }
        } else {
